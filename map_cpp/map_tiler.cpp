...
MapTiler::MapTiler (int map_w, int map_h, int tile_w, int tile_h, MapModel *model) :
    m_tile_w (tile_w),
    m_tile_h (tile_h),
    m_half_w (tile_w / 2),
    m_half_h (tile_h / 2),
    ...
    while (row < m_num_rows) {
        ...
        while (x + m_half_w <= map_w - m_x_min) {
            ...
            Point top (cx, cy - m_half_h);
            Point right (cx + m_half_w, cy);
            Point bottom (cx, cy + m_half_h);
            Point left (cx - m_half_w, cy);
