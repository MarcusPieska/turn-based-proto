...
lib.wander_down.restype = None
lib.wander_down.argtypes = [
    ctypes.POINTER(ctypes.c_int), 
    size, 
    pt, 
    ctypes.c_int, 
...
def wander_down_cpp (dist, start_pt, min_limit):
    ...
    lib.wander_down(dist_ptr, sz, start, min_limit, ctypes.byref(end))
...
if __name__ == "__main__":
    ...
    start_points, path_pixels_total = [], []
    start_points.append((360, 160))
    start_points.append((180, 120))
    start_points.append((430, 140))
    min_limit = 0
    ...
    for p in img_paths:
        ...
        for i, start_pt in enumerate(start_points):
            ...
            end_pt = wander_down_cpp(dist, start_pt, min_limit)
            t1 = time.perf_counter()
            path_pixels = generate_path_pixels(start_pt, end_pt, (w, h))
            path_pixels_total.extend(path_pixels)
            assert end_pt in path_pixels, "End point %s not in path pixels %s" % (end_pt, path_pixels)
            assert start_pt in path_pixels, "Start point %s not in path pixels %s" % (start_pt, path_pixels)
        out_img = np.zeros((h, w, 3), dtype=np.uint8)
        out_img[dist == 0] = [0, 0, 128]
        for px, py in path_pixels_total:
            if 0 <= px < w and 0 <= py < h:
                out_img[py, px] = [255, 255, 255]
        Image.fromarray(out_img, mode='RGB').save(p.replace(".png", "_wander_down_%d.png" % i))
