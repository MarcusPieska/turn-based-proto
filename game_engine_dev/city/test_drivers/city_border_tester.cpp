...
static const char* k_out = "/home/w/Projects/simple-map-gen/city-border-offsets/city_border_bands.ppm";
...
int main () {
    ...
    for (i32 cult = 65000; cult >= 0; --cult) {
        if (cult < 65000 && !CityBorder::will_expand(static_cast<u16>(cult + 1), static_cast<u16>(cult))) {
            continue;
        }
        const u16 radius = CityBorder::radius_for(static_cast<u16>(cult));
        const CircArea area = CityBorder::get(radius);
        if (area.m_lim == 0) {
            continue;
        }
        ...
        use_red = !use_red;
        ++valid_n;
        std::printf("painted cult=%d r=%u lim=%u\n", cult, (unsigned)radius, (unsigned)area.m_lim);
    }
