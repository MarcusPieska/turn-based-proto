//================================================================================================================================
//=> - WARNING -
//================================================================================================================================
//
//  - AUTO-GENERATED by generate_booster_registers.py
//  - Do not edit this file manually.
//
//================================================================================================================================
//=> - Includes -
//================================================================================================================================

#include "city_happiness_booster_register.h"

//================================================================================================================================
//=> - CityHappinessBoosterRegister data -
//================================================================================================================================

const BoosterRegisterEntry CityHappinessBoosterRegister::s_entry[12] = {
    { { EffectEnablerKind::BUILDING, 4 }, 1, 0 }, // Amphitheater (building)
    { { EffectEnablerKind::BUILDING, 5 }, 1, 0 }, // Temple (building)
    { { EffectEnablerKind::BUILDING, 13 }, 1, 0 }, // Public Fountains (building)
    { { EffectEnablerKind::BUILDING, 14 }, 1, 0 }, // Public Baths (building)
    { { EffectEnablerKind::BUILDING, 20 }, 2, 0 }, // Cathedral (building)
    { { EffectEnablerKind::BUILDING, 22 }, 2, 0 }, // Colosseum (building)
    { { EffectEnablerKind::BUILDING, 24 }, 1, 0 }, // Police Station (building)
    { { EffectEnablerKind::BUILDING, 33 }, 1, 0 }, // Sewage Pumps (building)
    { { EffectEnablerKind::BUILDING, 37 }, 1, 0 }, // Laundry (building)
    { { EffectEnablerKind::WONDER, 2 }, 3, 0 }, // Hanging Gardens (wonder)
    { { EffectEnablerKind::WONDER, 14 }, 3, 0 }, // Shakespeare's Theatre (wonder)
    { { EffectEnablerKind::WONDER, 16 }, 2, 0 }, // J.S. Bach's Cathedral (wonder)
};

//================================================================================================================================
//=> - End of file -
//================================================================================================================================
