...
void AssessorBruteWrite::emit_reqs (FILE* out, const InferredReqs& ir, const StaticParsingManager& mgr) {
    for (u8 i = 0; i < ir.m_tech_n; ++i) {
        u16 ix = ir.m_tech[i];
        if (ix < mgr.get_tech_count()) {
            emit_req_tok(out, "tech", mgr.get_tech_name_parser().idx_to_name(ix), 0);
        }
    }
    for (u8 i = 0; i < ir.m_resource_n; ++i) {
        u16 ix = ir.m_resource[i];
        if (ix < mgr.get_resource_count()) {
            emit_req_tok(out, "resource", mgr.get_resource_name_parser().idx_to_name(ix), 0);
        }
    }
    for (u8 i = 0; i < ir.m_building_n; ++i) {
        u16 ix = ir.m_building[i];
        if (ix < mgr.get_building_count()) {
