...
class City {
public:
    static GeneralBitBank* create_bank (u16 b_size) {
        return new GeneralBitBank(b_size);
    }
...
void set_all_flags_for_batch (GeneralBitBank& bank, u16 batch_idx, u16 b_size, bool value) {
...
void print_batch_flags (GeneralBitBank& bank, u16 batch_idx, u16 b_size) {
    ...
    for (u16 flag_idx = 0; flag_idx < b_size; ++flag_idx) {
        bool bit = City::is_flagged(bank, batch_idx, flag_idx);
        if (bit) {
...
void print_batch1_only (GeneralBitBank& bank, u16 b_size) {
...
void test_visual_walk_batch1_flags_for_sizes_1_to_100 () {
    ...
    for (u16 b_size = 1; b_size <= 100; ++b_size) {
        GeneralBitBank* bank = City::create_bank(b_size);
        ...
        for (u16 idx = 0; idx < b_size; ++idx) {
            ...
            u16 idx_batch1 = 0;
            for (u16 flag_idx = 0; flag_idx < b_size; ++flag_idx) {
                bool b0 = City::is_flagged(*bank, 0, flag_idx);
                bool b1 = City::is_flagged(*bank, 1, flag_idx);
                bool b2 = City::is_flagged(*bank, 2, flag_idx);
