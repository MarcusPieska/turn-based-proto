...
class ResourcesAvailableAccessor {
public:
    static ResourcesAvailableKey make_key (u16 raw) {
        return ResourcesAvailableKey(raw);
    }
};

void assert_all_clear (ResourcesAvailableArray& arr, u16 count, cstr prefix) {
    for (u16 i = 0; i < count; ++i) {
        ResourcesAvailableKey key = ResourcesAvailableAccessor::make_key(i);
...
void test_array_starts_clear () {
    ...
    ResourcesAvailableArray arr(flag_count);
    assert_all_clear(arr, flag_count, "ResourcesAvailableArray starts clear");
...
void test_set_flag_roundtrip () {
    const u16 flag_count = 16;
    ResourcesAvailableArray arr(flag_count);
    ResourcesAvailableKey key = ResourcesAvailableAccessor::make_key(5);
...
void test_multiple_flags_independent () {
    ...
    ResourcesAvailableKey key_a = ResourcesAvailableAccessor::make_key(2);
    ResourcesAvailableKey key_b = ResourcesAvailableAccessor::make_key(9);
    ResourcesAvailableKey key_c = ResourcesAvailableAccessor::make_key(13);
...
void test_none_key_access () {
    const u16 flag_count = 16;
    ResourcesAvailableArray arr(flag_count);
    ResourcesAvailableKey none_key = ResourcesAvailableKey::None();
...
int main (int argc, char* argv[]) {
    ...
    printf("=======================================================\n");
    printf(" TESTING ResourcesAvailable ARRAY: TOTAL FAILURES: %d/%d\n", total_test_fails, total_tests_run);
    printf("=======================================================\n");
