...
class BuildingsBuiltAccessor {
public:
    static BuildingsBuiltKey make_key (u16 raw) {
        return BuildingsBuiltKey(raw);
    }
};

void assert_all_clear (BuildingsBuiltArray& arr, u16 count, cstr prefix) {
    for (u16 i = 0; i < count; ++i) {
        BuildingsBuiltKey key = BuildingsBuiltAccessor::make_key(i);
...
void test_array_starts_clear () {
    ...
    BuildingsBuiltArray arr(flag_count);
    assert_all_clear(arr, flag_count, "BuildingsBuiltArray starts clear");
...
void test_set_flag_roundtrip () {
    const u16 flag_count = 16;
    BuildingsBuiltArray arr(flag_count);
    BuildingsBuiltKey key = BuildingsBuiltAccessor::make_key(5);
...
void test_multiple_flags_independent () {
    ...
    BuildingsBuiltKey key_a = BuildingsBuiltAccessor::make_key(2);
    BuildingsBuiltKey key_b = BuildingsBuiltAccessor::make_key(9);
    BuildingsBuiltKey key_c = BuildingsBuiltAccessor::make_key(13);
...
void test_none_key_access () {
    const u16 flag_count = 16;
    BuildingsBuiltArray arr(flag_count);
    BuildingsBuiltKey none_key = BuildingsBuiltKey::None();
...
int main (int argc, char* argv[]) {
    ...
    printf("=======================================================\n");
    printf(" TESTING BuildingsBuilt ARRAY: TOTAL FAILURES: %d/%d\n", total_test_fails, total_tests_run);
    printf("=======================================================\n");
