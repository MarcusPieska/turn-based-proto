...
class CivsLoadedAccessor {
public:
    static CivsLoadedKey make_key (u16 raw) {
        return CivsLoadedKey(raw);
    }
};

void assert_all_clear (CivsLoadedArray& arr, u16 count, cstr prefix) {
    for (u16 i = 0; i < count; ++i) {
        CivsLoadedKey key = CivsLoadedAccessor::make_key(i);
...
void test_array_starts_clear () {
    ...
    CivsLoadedArray arr(flag_count);
    assert_all_clear(arr, flag_count, "CivsLoadedArray starts clear");
...
void test_set_flag_roundtrip () {
    const u16 flag_count = 16;
    CivsLoadedArray arr(flag_count);
    CivsLoadedKey key = CivsLoadedAccessor::make_key(5);
...
void test_multiple_flags_independent () {
    ...
    CivsLoadedKey key_a = CivsLoadedAccessor::make_key(2);
    CivsLoadedKey key_b = CivsLoadedAccessor::make_key(9);
    CivsLoadedKey key_c = CivsLoadedAccessor::make_key(13);
...
void test_none_key_access () {
    const u16 flag_count = 16;
    CivsLoadedArray arr(flag_count);
    CivsLoadedKey none_key = CivsLoadedKey::None();
...
int main (int argc, char* argv[]) {
    ...
    printf("=======================================================\n");
    printf(" TESTING CivsLoaded ARRAY: TOTAL FAILURES: %d/%d\n", total_test_fails, total_tests_run);
    printf("=======================================================\n");
