...
void test_get_item_returns_expected_struct_by_key () {
    ...
    const [DATA_STRUCT]& item = data.get_item(key);
...
void test_take_ownership_copies_backing_array () {
    [DATA_STRUCT]* items = new [DATA_STRUCT][2];
    items[0].[STRUCT_MEMBER1] = 51; items[0].[STRUCT_MEMBER2] = 151;
    items[1].[STRUCT_MEMBER1] = 52; items[1].[STRUCT_MEMBER2] = 152;
    [CLASS_NAME] data;
    data.set_items(items, 2);
    [DATA_KEY] key = [CLASS_NAME]Tester::make_key(1);
    ...
    note_result(item.[STRUCT_MEMBER1] == 52, "take_ownership preserves member1");
    note_result(item.[STRUCT_MEMBER2] == 152, "take_ownership preserves member2");
    data.release_items();
    summarize_test_results();
}
