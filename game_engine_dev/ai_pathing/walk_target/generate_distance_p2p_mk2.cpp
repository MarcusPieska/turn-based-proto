...
#include "generate_distance_p2p_mk2.h"
...
static const u8 k_step_top = static_cast<u8>(Generate_DistanceP2P_Mk2::k_ovl_mod - 1u);
...
static void assign_steps_incr (
    ...
    for (u32 i = 0; i < tile_n; ++i) {
        su4[i] = static_cast<u8>(Generate_DistanceP2P_Mk2::k_step_sent);
    }
    su4[dst_i] = k_step_top;
    ...
    for (u32 qh = 0; qh < qn; ++qh) {
        const u32 pi = q[qh];
        const u16 px = static_cast<u16>(pi % static_cast<u32>(w));
        const u16 py = static_cast<u16>(pi / static_cast<u32>(w));
        for (u32 k = 0u; k < MAP_NBR4_N; ++k) {
            ...
            const u32 ci = tidx(w, tx, ty);
            if (turn[ci] == k_turn_none || pred[ci] != pi) {
                continue;
            }
            su4[ci] = step_from_parent(su4, turn, pi, ci, turn[ci]);
            if (vis[ci] == 0) {
                vis[ci] = 1;
                q[qn++] = ci;
            }
...
static void pack_overlays (
    ...
    assign_steps_incr(w, h, tile_n, dst_i, turn, pred, su4);
    for (u16 y = 0; y < h; ++y) {
        for (u16 x = 0; x < w; ++x) {
            const u32 i = tidx(w, x, y);
            if (turn[i] == k_turn_none) {
                turn_o->set(x, y, Generate_DistanceP2P_Mk2::k_turn_sent);
                step_o->set(x, y, Generate_DistanceP2P_Mk2::k_step_sent);
                continue;
            }
            turn_o->set(x, y, static_cast<u32>(turn[i] % Generate_DistanceP2P_Mk2::k_ovl_mod));
            step_o->set(x, y, static_cast<u32>(su4[i]));
        }
    }
    delete[] su4;
}
    ...
    *out_max = 0;
    ...
    while (q_cur.count() > 0u) {
        ...
        for (u32 qh = 0; qh < q_cur.count(); ++qh) {
            ...
            try_fan(terrain, rivers, w, h, si, sx, sy, bud, st, sq, q_nxt, turn, mp, step, pred, mp_z, out_max);
        }
        for (u32 sh = 0; sh < sq.count(); ++sh) {
            ...
            try_fan(terrain, rivers, w, h, si, sx, sy, bud, turn[si], sq, q_nxt, turn, mp, step, pred, mp_z, out_max);
...
//================================================================================================================================
//=> - Generate_DistanceP2P_Mk2 -
//================================================================================================================================

bool Generate_DistanceP2P_Mk2::generate (
    const u8* terrain,
    const u8* rivers,
    u16 w,
    u16 h,
    ...
    *out_max = 0;
    if (src_i == dst_i) {
        pack_overlays(w, h, tile_n, dst_i, turn_f, pred, turn_o, step_o);
        delete[] turn_f;
        delete[] step_f;
        return true;
    }
    const bool ok = flood_p2p(terrain, rivers, w, h, tile_n, src_i, turn_f, mp, step_f, pred, sq_spent, q_cur, q_nxt, sq, k_mp_z, out_max);
    if (ok) {
        pack_overlays(w, h, tile_n, dst_i, turn_f, pred, turn_o, step_o);
    } else {
        for (u16 y = 0; y < h; ++y) {
            for (u16 x = 0; x < w; ++x) {
                turn_o->set(x, y, k_turn_sent);
                step_o->set(x, y, k_step_sent);
            }
        }
        for (u32 i = 0; i < tile_n; ++i) {
            pred[i] = k_pred_none;
