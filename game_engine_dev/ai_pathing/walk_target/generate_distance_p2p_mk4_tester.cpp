...
static const cstr k_in_path = "/home/w/Projects/simple-map-gen/p1-seed-43/terrain.ppm";
static const cstr k_in_riv = "/home/w/Projects/simple-map-gen/p1-seed-43/rivers.ppm";
static const cstr k_out_path = "/home/w/Projects/simple-map-gen/distance-p2p-mk4.ppm";
static const cstr k_trace_path = "/home/w/Projects/simple-map-gen/distance-p2p-mk4.trace";
...
static bool audit_pass (const P2PLowerAudit& a) {
    return a.n_no_lower == 0u;
}
...
static bool ovl_dst (const u16* dist, u16 w, u16 x, u16 y) {
    return dist[tidx(w, x, y)] == 0u;
}

static bool find_lowest_lower_adj (
    u32 d,
    const u16* dist,
    u16 w,
    u16 h,
...
static P2PLowerAudit audit_lower_adj (
    const u16* dist,
    u16 w,
    u16 h) {
    P2PLowerAudit res = {};
    for (u16 y = 0; y < h; ++y) {
        for (u16 x = 0; x < w; ++x) {
            if (!ovl_reach(dist, w, x, y)) {
                continue;
            }
            res.n_val++;
            if (ovl_dst(dist, w, x, y)) {
                res.n_end++;
                continue;
            }
            ...
            u16 tx = x;
            u16 ty = y;
            if (find_lowest_lower_adj(d, dist, w, h, x, y, tx, ty)) {
                res.n_has_lower++;
            } else {
                res.n_no_lower++;
                if (!res.have_fail) {
...
static void dbg_no_lower_nbrs (
    const P2PLowerAudit& audit,
    const u16* dist,
    u16 w,
    u16 h) {
    if (!audit.have_fail) {
    ...
    const u16 x = audit.fail_x;
    const u16 y = audit.fail_y;
    const u32 d = static_cast<u32>(dist[tidx(w, x, y)]);
    std::printf("  no_lower nbrs at (%u,%u) d=%u:\n",
...
static void print_test_outcome (
    ...
    std::printf(
        "  overlay val=%u %shas_lower=%u%s %sno_lower=%u%s end=%u\n",
        audit.n_val,
        k_ansi_grn,
        audit.n_has_lower,
        k_ansi_rst,
        audit.n_no_lower == 0u ? k_ansi_grn : k_ansi_red,
        audit.n_no_lower,
        k_ansi_rst,
        audit.n_end);
    if (audit.have_fail) {
        std::printf("  first no_lower (%u,%u) dist=%u\n",
            static_cast<unsigned>(audit.fail_x),
            static_cast<unsigned>(audit.fail_y),
            static_cast<unsigned>(audit.fail_d));
    }
    std::printf("  %smemset time %.6f s%s\n",
        k_ansi_blu,
        memset_sec,
        k_ansi_rst);
...
static u8 norm_gray_dist (u16 d, u16 max_turn) {
    if (d >= k_dist_sent) {
        return 0;
    }
    if (max_turn == 0u) {
    ...
    return static_cast<u8>((static_cast<u32>(d) * 255u) / static_cast<u32>(max_turn));
}
...
static bool build_img (
    ...
    for (u32 i = 0; i < n; ++i) {
        ...
        if (is_water(t)) {
            ...
            g = k_wtr_g;
            b = k_wtr_b;
        } else if (dist[i] != k_dist_sent) {
            const u8 gry = norm_gray_dist(dist[i], max_turn);
            r = gry;
            g = gry;
            b = gry;
        } else {
...
int main () {
    ...
    u16* dist = new u16[tile_n];
    u32* pred = new u32[tile_n];
    const i32 tn_i = static_cast<i32>(tile_n);
    ...
    const clock_t tm0 = clock();
    std::memset(scr[0], 0, static_cast<size_t>(tile_n) * sizeof(u16));
    const double memset_sec = static_cast<double>(clock() - tm0) / static_cast<double>(CLOCKS_PER_SEC);
    u16 p2p_max = 0;
    TRACE_SETUP((k_trace_path));
    const clock_t t0 = clock();
    const bool ok = Generate_DistanceP2P_Mk4::generate(
        terrain, rivers, w, h, src_x, src_y, dst_x, dst_y, dist, pred, scr, &p2p_max);
    const double ovl_sec = static_cast<double>(clock() - t0) / static_cast<double>(CLOCKS_PER_SEC);
    if (!ok) {
        std::printf("*** FAILED p2p generate (no path)\n");
        return 1;
    }
    const P2PLowerAudit audit = audit_lower_adj(dist, w, h);
    u8* rgb = new u8[static_cast<size_t>(tile_n) * 3u];
    build_img(terrain, dist, w, h, src_x, src_y, dst_x, dst_y, p2p_max, rgb);
    if (!save_rgb_ppm(k_out_path, rgb, w, h)) {
        std::printf("*** FAILED save %s\n", k_out_path);
        return 1;
    }
    print_test_outcome(w, h, src_x, src_y, dst_x, dst_y, p2p_max, audit, ovl_sec, memset_sec, k_out_path);
    ...
    delete[] land_dep;
    if (!test_ok) {
        dbg_no_lower_nbrs(audit, dist, w, h);
        TRACE_PATH_FAILURE(("mk4 audit failed"));
        std::printf("  trace %s\n", k_trace_path);
        std::printf("*** FAILED generate_distance_p2p_mk4\n");
        return 1;
    }
    std::printf("*** PASSED generate_distance_p2p_mk4\n");
    return 0;
}
