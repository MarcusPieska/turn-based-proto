...
static const cstr k_out_path = "/home/w/Projects/simple-map-gen/distance-p2p-mk2.ppm";
...
static const u32 k_turn_sent = Generate_DistanceP2P_Mk2::k_turn_sent;
static const u32 k_ovl_mod = Generate_DistanceP2P_Mk2::k_ovl_mod;
static const u32 k_step_top = Generate_DistanceP2P_Mk2::k_ovl_mod - 1u;
...
static u32 turn_drop_u4 (u32 prev, u32 cur) {
    const u32 d = (prev + k_ovl_mod - cur) % k_ovl_mod;
    if (d >= 1u && d <= (k_ovl_mod / 2u)) {
        return d;
    }
    return 0u;
}
...
static bool trace_path (
    ...
    walk_turns = 0;
    traced = false;
    u32 prev_t = turn_o.get(px, py);
    for (u32 i = 0; i < tile_n; ++i) {
        path_m[i] = 0;
    }
    path_m[tidx(w, px, py)] = path_mark(rivers, w, px, py);
    for (u32 guard = 0; guard < tile_n; ++guard) {
        if (px == dst_x && py == dst_y) {
            traced = true;
            break;
        }
        if (!step_down(turn_o, step_o, pred, w, h, px, py)) {
            break;
        }
        const u32 cur_t = turn_o.get(px, py);
        walk_turns += turn_drop_u4(prev_t, cur_t);
        prev_t = cur_t;
        path_m[tidx(w, px, py)] = path_mark(rivers, w, px, py);
        path_len++;
    }
...
int main () {
    ...
    MapBitArrayOverlay turn_o(w, h, Generate_DistanceP2P_Mk2::k_turn_bpv);
    MapBitArrayOverlay step_o(w, h, Generate_DistanceP2P_Mk2::k_step_bpv);
    u32* pred = new u32[tile_n];
    u16* scr[Generate_DistanceP2P_Mk2::k_scr_n];
    for (u32 s = 0; s < Generate_DistanceP2P_Mk2::k_scr_n; ++s) {
        scr[s] = new u16[tile_n];
    }
    u16 p2p_max = 0;
    const clock_t t0 = clock();
    const bool ok = Generate_DistanceP2P_Mk2::generate(
        terrain, rivers, w, h, src_x, src_y, dst_x, dst_y, &turn_o, &step_o, pred, scr, g_mp_turn, &p2p_max);
    ...
    trace_path(turn_o, step_o, pred, rivers, w, h, src_x, src_y, dst_x, dst_y, path_m, path_len, walk_turns, traced);
    ...
    if (!test_ok) {
        std::printf("*** FAILED generate_distance_p2p_mk2\n");
        return 1;
    }
    std::printf("*** PASSED generate_distance_p2p_mk2\n");
    return 0;
}
