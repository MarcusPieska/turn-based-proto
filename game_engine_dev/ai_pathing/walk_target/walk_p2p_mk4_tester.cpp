...
static WalkSimRes sim_walk (
    ...
    while (px != dx || py != dy) {
        if (++guard > tile_n) {
            TRACE_PATH_FAILURE(("mk4 walk guard"));
            break;
        }
        while (px != dx || py != dy) {
            const WalkP2P_Mk4::StepRes st = wk.peek_step(
                dist, terrain, rivers, w, h, px, py);
            if (!st.have) {
                TRACE_P2P_MK4_WALK_STALL((
                    px, py, static_cast<u16>(mp > 0 ? mp : 0), turn));
                TRACE_PATH_FAILURE(("mk4 walk stall"));
                res.game_turns = turn;
                return res;
            }
        ...
        if (px == dx && py == dy) {
            res.traced = true;
            TRACE_P2P_MK4_WALK_DONE((dx, dy, res.path_len, turn));
            break;
        }
        turn++;
...
static void print_outcome (
    ...
    std::printf("test walk p2p mk4: %s\n", pass ? "PASS" : "FAIL");
...
int main () {
    ...
    delete[] land_dep;
    if (!res.traced) {
        TRACE_PATH_FAILURE(("mk4 walk trace failed"));
        std::printf("  trace %s\n", k_trace_path);
        std::printf("*** FAILED walk_p2p_mk4\n");
        return 1;
    }
    std::printf("*** PASSED walk_p2p_mk4\n");
    return 0;
}
