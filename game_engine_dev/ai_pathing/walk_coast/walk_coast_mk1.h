...
//================================================================================================================================
//
// - Coastal explorer mk1: greedy walk on coast-adjacent land, then local repos via gradient
// - Greedy phase picks the neighbor that reveals the most unexplored tiles in sight
// - Repos phase flood-fills a 31x31 window toward unexplored coastal-front fog, then descends
// - Function: can_ai_start_from: 0=no, 1=walk (walk-front + pick), 2=repos (path + grad fog)
...
public:
    ...
    WalkCoastMk1 (const GameArraySimple& map, MapBitOverlay& exp, u16 sx, u16 sy, u16 sight, u8 player);
    void move (u16 moves);
    u16 x () const;
    u16 y () const;
    u8 phase () const;
...
private:
    static const u8 k_ph_walk = 0u; // Greedy coastal walk
    ...
    static const u8 k_wat = 253u; // Interior in m_dst: unexplored path off coast front; BFS fills dist
