...
static const cstr WC_START_MAP = "/home/w/Projects/simple-map-gen/walk_coast_mk1_start_map.ppm";
static const cstr WC_OUT = "/home/w/Projects/simple-map-gen/walk_coast_mk1_start_result.ppm";
static const cstr WC_FRAMES = "/home/w/Projects/simple-map-gen/walk_coast_mk1_start_frames";
...
static void classify_starts (
    ...
    for (u16 y = 0u; y < h; ++y) {
        for (u16 x = 0u; x < w; ++x) {
            ...
            const u16 st = WalkCoastMk1::can_ai_start_from(map, exp, x, y, sight);
...
static void mark_unit (u8* rgb, u16 w, u16 h, const WalkCoastMk1& ai) {
    if (ai.phase() == 0u) {
...
int main (int argc, char** argv) {
    ...
    classify_starts(map, exp, WC_SIGHT, ov, n_land, n_walk, n_repo, prof_us, prof_n);
    ...
    u8* terr_rgb = nullptr;
    if (!load_terr_rgb(WC_IN_TERR, map_w, map_h, &terr_rgb)) {
        t_fail("*** FAILED load terrain rgb\n");
        delete[] ov;
        return 1;
    }
    if (!save_start_map(terr_rgb, ov, map_w, map_h, WC_START_MAP)) {
    ...
    if (verbose) {
        std::printf("walk_coast_mk1_start: %ux%u sight %u\n",
    ...
    WalkCoastMk1 ai(map, exp, spawn_x, spawn_y, WC_SIGHT, 0u);
    if (ai.phase() != 1u) {
    ...
    if (!save_frame_ppm(terr_rgb, exp, map_w, map_h, spawn_x, spawn_y, ai, WC_OUT)) {
        t_fail("*** FAILED save %s\n", WC_OUT);
        delete[] terr_rgb;
        delete[] ov;
        return 1;
    ...
    std::printf("result: %s\n", WC_OUT);
    if (verbose) {
        t_pass("*** PASSED walk_coast_mk1_start\n");
    }
    delete[] terr_rgb;
    delete[] ov;
    delete[] prof_us;
