...
static const cstr NE_OUT = "/home/w/Projects/simple-map-gen/walk_near_mk1_adv_result.ppm";
...
static const char* bias_name (WalkNearBias b) {
    if (b == WN_BIAS_NORTH) {
        return "north";
    }
    if (b == WN_BIAS_SOUTH) {
...
int main (int argc, char** argv) {
    ...
    const WalkNearBias bias[NE_UNIT_N] = {
        WN_BIAS_NORTH,
        WN_BIAS_SOUTH,
        WN_BIAS_EAST,
        WN_BIAS_WEST
    ...
    WalkNearMk1 ai_n(map, ov, ux, uy, NE_SIGHT, 0u, WN_BIAS_NORTH);
    WalkNearMk1 ai_s(map, ov, ux, uy, NE_SIGHT, 1u, WN_BIAS_SOUTH);
    WalkNearMk1 ai_e(map, ov, ux, uy, NE_SIGHT, 2u, WN_BIAS_EAST);
    WalkNearMk1 ai_w(map, ov, ux, uy, NE_SIGHT, 3u, WN_BIAS_WEST);
    WalkNearMk1* ai[NE_UNIT_N] = {&ai_n, &ai_s, &ai_e, &ai_w};
    char path[160];
    u32 frame = 0u;
    std::snprintf(path, sizeof(path), "%s/%05u.ppm", NE_FRAMES, static_cast<unsigned>(frame));
    if (!save_adv_ppm(map, ov, ai, ur, ug, ub, path)) {
        std::printf("*** FAILED save frame %u\n", static_cast<unsigned>(frame));
        return 1;
    }
    if (verbose) {
        std::printf("walk_near_mk1_adv: %ux%u sight %u home (%u,%u)\n",
    ...
    if (!save_adv_ppm(map, ov, ai, ur, ug, ub, NE_OUT)) {
    ...
    std::printf("result: %s\n", NE_OUT);
    if (verbose) {
        std::printf("*** PASSED walk_near_mk1_adv\n");
    }
    return 0;
}
