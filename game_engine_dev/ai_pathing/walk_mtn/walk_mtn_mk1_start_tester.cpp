...
static const cstr WM_START_MAP = "/home/w/Projects/simple-map-gen/walk_mtn_mk1_start_map.ppm";
static const cstr WM_OUT = "/home/w/Projects/simple-map-gen/walk_mtn_mk1_start_result.ppm";
static const cstr WM_FRAMES = "/home/w/Projects/simple-map-gen/walk_mtn_mk1_start_frames";
static u16 WM_TURNS = 0;
...
static void mark_unit (u8* rgb, u16 w, u16 h, const WalkMtnMk1& ai) {
    if (ai.phase() == 0u) {
...
int main (int argc, char** argv) {
    ...
    WM_TURNS = g_rt_statics->config().get_mov_pt_per_turn();
    GameArraySimple map;
    ...
    classify_starts(map, exp, WM_SIGHT, ov, n_land, n_walk, n_repo, prof_us, prof_n);
    ...
    u8* terr_rgb = nullptr;
    if (!load_terr_rgb(WM_IN_TERR, map_w, map_h, &terr_rgb)) {
        t_fail("*** FAILED load terrain rgb\n");
        delete[] ov;
        return 1;
    }
    if (!save_start_map(terr_rgb, ov, map_w, map_h, WM_START_MAP)) {
    ...
    if (verbose) {
        std::printf("walk_mtn_mk1_start: %ux%u sight %u\n",
    ...
    WalkMtnMk1 ai(map, exp, spawn_x, spawn_y, WM_SIGHT, 0u);
    if (ai.phase() != 1u) {
    ...
    if (!save_frame_ppm(terr_rgb, exp, map_w, map_h, spawn_x, spawn_y, ai, WM_OUT)) {
        t_fail("*** FAILED save %s\n", WM_OUT);
        delete[] terr_rgb;
        delete[] ov;
        return 1;
    ...
    std::printf("result: %s\n", WM_OUT);
    if (verbose) {
        t_pass("*** PASSED walk_mtn_mk1_start\n");
    }
    delete[] terr_rgb;
    delete[] ov;
    delete[] prof_us;
