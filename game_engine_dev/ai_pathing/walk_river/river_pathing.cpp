...
static bool is_path (const GameArraySimple& map, u16 x, u16 y) {
...
static bool can_step (const GameArraySimple& map, u16 x, u16 y) {
    return is_path(map, x, y) || RiverPathing::is_riv(map, x, y);
}

bool RiverPathing::find_land_path_to_river (
    const GameArraySimple& map,
    const MapBitOverlay& exp,
    u16 sx,
    u16 sy,
