...
static const cstr RE_OUT = "/home/w/Projects/simple-map-gen/walk_river_mk2_result.ppm";
static const cstr RE_SYS_MAP = "/home/w/Projects/simple-map-gen/walk_river_mk2_sysmap.ppm";
static const cstr RE_BAD_MAP = "/home/w/Projects/simple-map-gen/walk_river_mk2_badmap.ppm";
...
static void tr_greedy (
    ...
    u32& gs_n) {
    if (bg && !ag) {
        tr_ge(bx, by, ge_x, ge_y, ge_n);
    }
    if (!bg && ag) {
        tr_gs(ax, ay, gs_x, gs_y, gs_n);
    }
}

static void move1_tr (
    WalkRiverMk2& ai,
    u16* ge_x,
    u16* ge_y,
    u32& ge_n,
    ...
    u16* gs_y,
    u32& gs_n) {
    if (ai.phase() == RE_PH_DONE) {
        return;
    }
    ai.reveal();
    bool acted = false;
    u16 loop = 0u;
    while (!acted && ai.phase() != RE_PH_DONE && loop < 128u) {
        const bool bg = ai.greedy_ph();
        const u16 bx = ai.x();
        const u16 by = ai.y();
        acted = ai.pump();
        tr_greedy(bg, ai.greedy_ph(), bx, by, ai.x(), ai.y(),
            ge_x, ge_y, ge_n, gs_x, gs_y, gs_n);
        ++loop;
    }
    ai.finish_move_credit(loop, acted);
}
...
int main (int argc, char** argv) {
    ...
    u32 gs_n = 0;
    if (verbose) {
        std::printf("walk_river_mk2: %ux%u turns %u moves %u sight %u\n",
    ...
    while (sys_n < RE_SYS_MAX && finder.find_next(ux, uy)) {
        ...
        WalkRiverMk2 ai(map, exp, ux, uy, RE_SIGHT, 0u);
        exp.set(ux, uy);
        vis.inc(ux, uy);
        TRACE_EXPLORE_DISCOVER((ux, uy, 0u));
        if (ai.greedy_ph()) {
            tr_gs(ai.x(), ai.y(), gs_x, gs_y, gs_n);
        }
        u16 sys_turn = 0;
        u8 last_ph = ai.phase();
        for (u16 t = 0; t < RE_TURNS && ai.phase() != RE_PH_DONE; ++t) {
            ...
            for (u16 m = 0; m < RE_MOVES && ai.phase() != RE_PH_DONE; ++m) {
                move1_tr(ai, ge_x, ge_y, ge_n, gs_x, gs_y, gs_n);
                vis.inc(ai.x(), ai.y());
            }
    ...
    if (sys_n == 0u) {
        t_fail("*** FAILED find river start\n");
        return 1;
    }
