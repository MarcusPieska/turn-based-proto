...
static void profile_can_ai_start (
    ...
    for (u16 y = 0u; y < h; ++y) {
        for (u16 x = 0u; x < w; ++x) {
            ...
            (void)WalkRiverMk1::can_ai_start_from(map, exp, x, y, sight);
...
static void mark_unit (u8* rgb, u16 w, u16 h, const WalkRiverMk1& ai) {
...
int main (int argc, char** argv) {
    ...
    if (verbose) {
        std::printf("walk_river_mk1_start: %ux%u sight %u\n",
    ...
    WalkRiverMk1 ai(map, sim_exp, spawn_x, spawn_y, WR_SIGHT, 0u);
    ...
    std::printf("result: %s\n", WR_OUT);
    if (verbose) {
        t_pass("*** PASSED walk_river_mk1_start\n");
    }
    delete[] terr_rgb;
    delete[] ov;
    delete[] prof_us;
