...
static const cstr WR_IN_TERR = "/home/w/Projects/simple-map-gen/p1-seed-042/24_make_map_terrain.ppm";
static const cstr WR_IN_CLIM = "/home/w/Projects/simple-map-gen/p1-seed-042/24_make_map_climate.ppm";
static const cstr WR_IN_RIV = "/home/w/Projects/simple-map-gen/p1-seed-042/24_make_map_rivers.ppm";
static const cstr WR_START_MAP = "/home/w/Projects/simple-map-gen/walk_river_mk2_start_map.ppm";
static const cstr WR_OUT = "/home/w/Projects/simple-map-gen/walk_river_mk2_start_result.ppm";
static const cstr WR_FRAMES = "/home/w/Projects/simple-map-gen/walk_river_mk2_start_frames";
static const u16 WR_TURNS = PATH_MP_TURN;
static const u16 WR_SIGHT = 3u;
static const u8 WR_PH_DONE = 3u;
static const u8 WR_PH_REPOS = 4u;
...
static void profile_can_ai_start (
    ...
    for (u16 y = 0u; y < h; ++y) {
        for (u16 x = 0u; x < w; ++x) {
            ...
            (void)WalkRiverMk2::can_ai_start_from(map, exp, x, y, sight);
...
static bool is_path_tile (const GameArraySimple& map, u16 x, u16 y) {
...
static bool can_step_tile (const GameArraySimple& map, u16 x, u16 y) {
    return is_path_tile(map, x, y) || RiverPathing::is_riv(map, x, y);
}

static void mark_repo_reach (
    const GameArraySimple& map,
    MapBitOverlay& exp,
    u16 sight,
    u8* reach) {
    ...
    for (u16 y = 0u; y < h; ++y) {
        for (u16 x = 0u; x < w; ++x) {
            if (!RiverPathing::is_riv_front(map, exp, x, y, sight)) {
                continue;
            }
            const u32 i = tidx(w, x, y);
            if (reach[i] != 0u) {
    ...
    for (u32 qh = 0u; qh < qn; ++qh) {
        ...
        for (u32 k = 0u; k < MAP_NBR8_N; ++k) {
            const i32 nx = static_cast<i32>(px) + MAP_NBR8_DX[k];
            const i32 ny = static_cast<i32>(py) + MAP_NBR8_DY[k];
            if (nx < 0 || ny < 0) {
                continue;
            }
            const u16 cx = static_cast<u16>(nx);
...
static void classify_starts (
    ...
    const u16 h = map.height();
    const u32 tile_n = map.tile_n();
    u8* reach = new u8[tile_n];
    if (reach == nullptr) {
        return;
    }
    mark_repo_reach(map, exp, sight, reach);
    n_land = 0u;
    n_walk = 0u;
    n_repo = 0u;
    for (u16 y = 0u; y < h; ++y) {
        for (u16 x = 0u; x < w; ++x) {
            ...
            if (is_path_tile(map, x, y) && !RiverPathing::is_riv(map, x, y) && reach[i] != 0u) {
                if (WalkRiverMk2::can_ai_start_from(map, exp, x, y, sight) == 2u) {
                    ov[i] = k_ov_repo;
                    ++n_repo;
                    continue;
                }
...
static bool find_repo_spawn (const GameArraySimple& map, const u8* ov, u16& ox, u16& oy) {
    const u16 w = map.width();
    const u16 h = map.height();
    const u16 cx = w / 2u;
    const u16 cy = h / 2u;
...
static void mark_unit (u8* rgb, u16 w, u16 h, const WalkRiverMk2& ai) {
    if (ai.phase() == WR_PH_DONE) {
...
static bool save_frame_ppm (
    ...
    mark_cross(rgb, w, h, sx, sy, 255, 255, 255);
...
int main (int argc, char** argv) {
    ...
    profile_can_ai_start(map, exp, WR_SIGHT, ov, prof_us, prof_n);
    prof_print(prof_us, prof_n);
    u32 n_land = 0u;
    u32 n_walk = 0u;
    u32 n_repo = 0u;
    classify_starts(map, exp, WR_SIGHT, ov, n_land, n_walk, n_repo);
    u32 n_off = 0u;
    for (u32 i = 0u; i < tile_n; ++i) {
        if (ov[i] == k_ov_off) {
            ++n_off;
    ...
    u8* terr_rgb = nullptr;
    if (!load_terr_rgb(WR_IN_TERR, map_w, map_h, &terr_rgb)) {
        t_fail("*** FAILED load terrain rgb\n");
        delete[] ov;
        return 1;
    }
    if (!save_start_map(terr_rgb, ov, map_w, map_h, WR_START_MAP)) {
    ...
    u16 spawn_x = 0u;
    u16 spawn_y = 0u;
    if (!find_repo_spawn(map, ov, spawn_x, spawn_y)) {
        t_fail("*** FAILED find repo spawn\n");
        delete[] terr_rgb;
        delete[] ov;
        return 1;
    }
    if (verbose) {
        std::printf("walk_river_mk2_start: %ux%u sight %u\n",
            static_cast<unsigned>(map_w),
            static_cast<unsigned>(map_h),
            static_cast<unsigned>(WR_SIGHT));
        std::printf("  landmass seed (%u,%u)\n",
            static_cast<unsigned>(seed_x),
            static_cast<unsigned>(seed_y));
        std::printf("  overlay off %u land %u walk %u repo %u\n",
            static_cast<unsigned>(n_off),
            static_cast<unsigned>(n_land),
            static_cast<unsigned>(n_walk),
            static_cast<unsigned>(n_repo));
        std::printf("  repo spawn (%u,%u)\n",
            static_cast<unsigned>(spawn_x),
            static_cast<unsigned>(spawn_y));
    }
    if (!ensure_dir(WR_FRAMES)) {
        t_fail("*** FAILED mkdir %s\n", WR_FRAMES);
        delete[] terr_rgb;
        delete[] ov;
        return 1;
    }
    MapBitOverlay sim_exp(map_w, map_h);
    WalkRiverMk2 ai(map, sim_exp, spawn_x, spawn_y, WR_SIGHT, 0u);
    if (ai.phase() != WR_PH_REPOS) {
        t_fail("*** FAILED repo spawn not in repos phase (ph %u)\n",
    ...
    char path[160];
    frame_path(path, sizeof(path), WR_FRAMES, 0u);
    if (!save_frame_ppm(terr_rgb, sim_exp, map_w, map_h, spawn_x, spawn_y, ai, path)) {
    ...
    u16 turn = 0u;
    u32 steps = 0u;
    while (turn < WR_TURNS && ai.phase() != WR_PH_DONE) {
        ++turn;
        const u16 ox = ai.x();
        const u16 oy = ai.y();
        ai.move(1u);
        ...
        frame_path(path, sizeof(path), WR_FRAMES, turn);
        if (!save_frame_ppm(terr_rgb, sim_exp, map_w, map_h, spawn_x, spawn_y, ai, path)) {
    ...
    if (!save_frame_ppm(terr_rgb, sim_exp, map_w, map_h, spawn_x, spawn_y, ai, WR_OUT)) {
        t_fail("*** FAILED save %s\n", WR_OUT);
        delete[] terr_rgb;
        delete[] ov;
        return 1;
    ...
    std::printf("turns %u steps %u frames %u\n",
        ...
        static_cast<unsigned>(turn));
    std::printf("end (%u,%u) phase %u done %u\n",
        static_cast<unsigned>(ai.x()),
        static_cast<unsigned>(ai.y()),
        static_cast<unsigned>(ai.phase()),
        static_cast<unsigned>(ai.phase() == WR_PH_DONE ? 1u : 0u));
    std::printf("frames: %s\n", WR_FRAMES);
    std::printf("result: %s\n", WR_OUT);
    if (verbose) {
        t_pass("*** PASSED walk_river_mk2_start\n");
    }
    delete[] terr_rgb;
    delete[] ov;
    delete[] prof_us;
