...
#include "eval_army_unit_support.h"
...
void EVAL_ARMY_UNIT_SUPPORT::EVAL (GameState& state) {
    ...
    for (u16 p = 0; p < state.m_player_n; ++p) {
        ...
        for (u16 i = 0; i < cn; ++i) {
            ...
            local_raw += static_cast<u32>(c->calc_city_land_unit_support(i));
            local_rem += static_cast<u32>(c->get_free_land_unit_support());
        }
        const u32 local_used = local_raw > local_rem ? local_raw - local_rem : 0u;
        const u32 civ_free = static_cast<u32>(ps.m_free_land_unit_support);
        const u32 unused_civ = static_cast<u32>(ps.m_target_new_land_unit_support);
        const u32 civ_used = civ_free > unused_civ ? civ_free - unused_civ : 0u;
        const u32 paid = static_cast<u32>(ps.m_land_unit_upkeep_needed);
        u32 units = 0u;
        for (u32 idx = 0; idx < scan_n; ++idx) {
            ...
            if (state.m_statics->unit_type_action_map().unit_type_can_do(us.type, static_cast<u16>(UnitAction::isLandUnit))) {
                ++units;
            }
        }
        TraceSink::printf(
            "army support seat=%u local_free=%u local_used=%u civ_free=%u civ_used=%u paid=%u units=%u\n",
