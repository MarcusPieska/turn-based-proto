...
i32 test_p1_gen_outline_basic (const P1_RunPrm& prm) {
    char out_path[320];
    if (!p1_tester_make_out(prm.m_seed, out_path, sizeof(out_path))) {
        std::printf("failed to ensure output dir\n");
        return -1;
    }
    P1_Gen_Outline gen(prm);
    const clock_t t0 = clock();
    const bool ok = gen.generate();
    const clock_t t1 = clock();
    const double sec = static_cast<double>(t1 - t0) / static_cast<double>(CLOCKS_PER_SEC);
    if (!ok || !gen.is_valid()) {
        std::printf("P1_Gen_Outline failed to generate\n");
        return -1;
    }
    const P1_Gen_OutlineRslt& r = gen.result();
    std::printf("P1_Gen_Outline generate time: %.6f s (%u x %u)\n",
        sec,
...
i32 main (i32 argc, char* argv[]) {
    ...
    return test_p1_gen_outline_basic(prm);
}
