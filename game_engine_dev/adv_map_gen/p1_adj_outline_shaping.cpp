...
static bool shape_outline_terrain (
    u32 seed,
    u16 w,
    u16 h,
    f32 grad_lim,
    ...
    bool first = true;
    for (u32 i = 0; i < n; ++i) {
        combo[i] -= land_dist_sub(land_depth[i], d_max, grad_lim);
        const f32 v = combo[i];
        if (first) {
            vmin = vmax = v;
            first = false;
...
P1_Adj_OutlineShaping::P1_Adj_OutlineShaping (const P1_RunPrm& prm, const P1_Adj_OutlineShapingPrm& sp) :
    m_prm(prm),
    m_sp(sp),
    m_valid_adjust(false) {
}

bool P1_Adj_OutlineShaping::adjust (u8* terrain, u16 w, u16 h, const u8* ol_ov, const u16* land_depth) {
    m_valid_adjust = false;
    ...
    if (!shape_outline_terrain(m_prm.m_seed, w, h, m_sp.m_inner_grad_limit, terrain, ol_ov, land_depth)) {
        return false;
    }
    m_valid_adjust = true;
    return true;
...
bool P1_Adj_OutlineShaping::is_valid () const {
