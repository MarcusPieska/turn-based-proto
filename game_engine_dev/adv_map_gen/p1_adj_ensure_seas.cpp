...
static bool adj8_coastal (const u8* terrain, u16 w, u16 h, u16 x, u16 y) {
...
static bool apply_ensure_seas (u8* terrain, u16 w, u16 h) {
    ...
    for (u16 y = 0; y < h; ++y) {
        for (u16 x = 0; x < w; ++x) {
            ...
            if (adj8_coastal(terrain, w, h, x, y)) {
                terrain[i] = TERR_SEA[0];
            }
        }
    }
    return true;
...
P1_Adj_EnsureSeas::P1_Adj_EnsureSeas (const P1_RunPrm& prm) :
    m_prm(prm),
    m_valid_adjust(false) {
}

bool P1_Adj_EnsureSeas::adjust (u8* terrain, u16 w, u16 h) {
    m_valid_adjust = false;
    ...
    if (!apply_ensure_seas(terrain, w, h)) {
        return false;
    }
    m_valid_adjust = true;
    return true;
...
bool P1_Adj_EnsureSeas::is_valid () const {
