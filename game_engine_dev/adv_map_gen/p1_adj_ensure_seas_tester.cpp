...
i32 test_p1_adj_ensure_seas_basic (P1_TesterHarness& h, const P1_Adj_LandAltitudePrm& lap) {
    char out_path[320];
    ...
    if (!h.run_input(&lap)) {
        std::printf("P1 steps 1-18 input failed for step 19\n");
        return -1;
    }
    P1_TesterChain15Rslt& chain = h.c15_mut();
    P1_Adj_EnsureSeas adj(h.prm());
    const clock_t t0 = clock();
    const bool ok = adj.adjust(chain.m_terrain, chain.m_w, chain.m_h);
    const clock_t t1 = clock();
    const double sec = static_cast<double>(t1 - t0) / static_cast<double>(CLOCKS_PER_SEC);
    if (!ok || !adj.is_valid()) {
        std::printf("P1_Adj_EnsureSeas failed to adjust\n");
        return -1;
    }
    std::printf("P1 steps 1-18 input time: %.6f s\n", h.input_sec());
    std::printf("P1_Adj_EnsureSeas adjust time: %.6f s (%u x %u)\n",
        sec,
...
i32 main (i32 argc, char* argv[]) {
    ...
    const i32 rc = test_p1_adj_ensure_seas_basic(h, lap);
    if (rc != 0) {
        return rc;
    }
    if (!h.finish()) {
