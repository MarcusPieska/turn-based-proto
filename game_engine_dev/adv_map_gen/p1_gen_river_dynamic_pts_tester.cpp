...
i32 test_p1_gen_river_dynamic_pts_basic (P1_TesterHarness& h) {
    if (!h.run_input()) {
    ...
    P1_Gen_RiverProb prob_gen(h.prm());
    if (!prob_gen.generate(ter, w, ht) || !prob_gen.is_valid()) {
        P1_RPrint::rprint_info("Failed to generate river prob");
        return -1;
    }
    ...
    P1_Gen_RiverDynamicPts gen(h.prm());
    const clock_t t0 = clock();
    const bool ok = gen.generate(ter, w, ht, prob_gen.result(), ocn_ref);
    const clock_t t1 = clock();
    const double sec = static_cast<double>(t1 - t0) / static_cast<double>(CLOCKS_PER_SEC);
    const P1_Gen_RiverPtsRslt& r = gen.result();
    if (!ok || !gen.is_valid() || r.m_n == 0u) {
        P1_RPrint::rprint_result_u32("P1_Gen_RiverDynamicPts", "Pts", r.m_n, false);
        return -1;
    }
    char tbuf[64];
    std::snprintf(tbuf, sizeof(tbuf), "Timing: %.6f s", sec);
    P1_RPrint::rprint_info(tbuf);
    P1_RPrint::rprint_state_u32("P1_Gen_RiverDynamicPts", "Pts", r.m_n);
    P1_RPrint::rprint_state("P1_Gen_RiverDynamicPts", "OcnSecs", r.m_ocn_sec_n);
...
i32 main (i32 argc, char* argv[]) {
    ...
    const i32 rc = test_p1_gen_river_dynamic_pts_basic(h);
    if (rc != 0) {
        return rc;
    }
    if (!h.finish()) {
