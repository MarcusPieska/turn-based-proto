...
i32 test_p1_gen_watershed_mountains_basic (const P1_RunPrm& prm) {
    char out_path[320];
    ...
    if (!build_step11_input(prm, terrain, w, h, &pts_rslt, &lin_gen, &net_gen)) {
        std::printf("P1 steps 1-13 input failed for step 14\n");
        delete[] terrain;
        return -1;
    }
    ...
    const bool ok = gen.generate(terrain, w, h, net_gen.result(), pts_rslt, noise);
    const clock_t t1 = clock();
    const double sec_i = static_cast<double>(t1i - t0i) / static_cast<double>(CLOCKS_PER_SEC);
    const double sec = static_cast<double>(t1 - t0) / static_cast<double>(CLOCKS_PER_SEC);
    if (!ok || !gen.is_valid()) {
        std::printf("P1_Gen_WatershedMountains failed to generate\n");
        delete[] terrain;
        return -1;
    }
    ...
    std::printf(
        "P1_Gen_WatershedMountains generate time: %.6f s (%u segments, %u border tiles, %u x %u)\n",
        sec,
        static_cast<u32>(r.m_seg_n),
        static_cast<u32>(r.m_border_tile_n),
        static_cast<u32>(w),
        static_cast<u32>(h));
    for (u16 si = 0; si < r.m_seg_n; ++si) {
        const P1_WatershedBorderSeg& s = r.m_segs[si];
        std::printf(
            "  seg %u basins %u-%u mouths (%u,%u)-(%u,%u) dist %u tiles %u a pln/hil %u/%u b pln/hil %u/%u\n",
            si,
            s.m_basin_a,
            s.m_basin_b,
            s.m_mouth_ax,
...
i32 main (i32 argc, char* argv[]) {
    ...
    const i32 rc = test_p1_gen_watershed_mountains_basic(prm);
    p1_wb_term();
    return rc;
}
