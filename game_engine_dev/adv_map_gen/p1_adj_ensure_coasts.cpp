...
static bool is_open_water (u8 cls) {
    return cls == TERR_OCEAN[0] || cls == TERR_SEA[0];
}

static bool adj8_land (const u8* terrain, u16 w, u16 h, u16 x, u16 y) {
...
static bool apply_ensure_coasts (u8* terrain, u16 w, u16 h) {
    ...
    for (u16 y = 0; y < h; ++y) {
        for (u16 x = 0; x < w; ++x) {
            ...
            if (!is_open_water(terrain[i])) {
                continue;
            }
            if (adj8_land(terrain, w, h, x, y)) {
...
P1_Adj_EnsureCoasts::P1_Adj_EnsureCoasts (const P1_RunPrm& prm) :
    m_prm(prm),
    m_valid_adjust(false) {
}

bool P1_Adj_EnsureCoasts::adjust (u8* terrain, u16 w, u16 h) {
    m_valid_adjust = false;
    ...
    if (!apply_ensure_coasts(terrain, w, h)) {
        return false;
    }
    m_valid_adjust = true;
    return true;
...
bool P1_Adj_EnsureCoasts::is_valid () const {
