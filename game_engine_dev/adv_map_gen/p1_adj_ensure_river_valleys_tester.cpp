...
i32 test_p1_adj_ensure_river_valleys_basic (const P1_RunPrm& prm, const P1_Adj_LandAltitudePrm& lap) {
    char out_path[320];
    ...
    P1_Adj_EnsureRiverValleys adj(prm);
    const clock_t t0 = clock();
    const bool ok = adj.adjust(chain.m_terrain, chain.m_w, chain.m_h, chain.m_river);
...
i32 main (i32 argc, char* argv[]) {
    ...
    return test_p1_adj_ensure_river_valleys_basic(prm, lap);
}
