...
i32 test_p1_gen_river_lines_basic (const P1_RunPrm& prm) {
    char out_path[320];
    ...
    if (!p1_gen_step01_ov(prm, &ov_map)) {
        std::printf("P1_Gen_ContOutlines failed for step 11 input\n");
        return -1;
    }
    ...
    if (!depth_gen.generate(ov, w, h) || !depth_gen.is_valid()) {
        std::printf("P1_Gen_LandDepth failed for step 11 input\n");
        return -1;
    }
    ...
    const bool ok = lin_gen.generate(terrain, w, h, sec_gen.result(), net_gen.result());
    const clock_t t1 = clock();
    const double sec_i = static_cast<double>(t1i - t0i) / static_cast<double>(CLOCKS_PER_SEC);
    const double sec = static_cast<double>(t1 - t0) / static_cast<double>(CLOCKS_PER_SEC);
    if (!ok || !lin_gen.is_valid()) {
        std::printf("P1_Gen_RiverLines failed to generate\n");
        delete[] terrain;
        return -1;
    }
    const P1_Gen_RiverLinesRslt& r = lin_gen.result();
    std::printf("P1 steps 1-10 input time: %.6f s\n", sec_i);
    std::printf("P1_Gen_RiverLines generate time: %.6f s (%u systems, %u x %u)\n",
        sec,
        static_cast<u32>(r.m_sys_n),
        static_cast<u32>(w),
        static_cast<u32>(h));
    lin_gen.save_output(out_path, terrain);
    delete[] terrain;
    std::printf("saved: %s\n", out_path);
    return 0;
}
...
i32 main (i32 argc, char* argv[]) {
    ...
    return test_p1_gen_river_lines_basic(prm);
}
