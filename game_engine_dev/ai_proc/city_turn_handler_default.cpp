...
static void pick_prod (GameState& state, u16 city_idx, City* city) {
    GAME_EXPECT(city != nullptr, "CityTurnHandler_Default pick_prod got nullptr city");
    GAME_EXPECT(state.m_statics != nullptr, "CityTurnHandler_Default pick_prod got nullptr statics");
    GAME_EXPECT(state.m_player_states != nullptr, "CityTurnHandler_Default pick_prod got nullptr player states");

    const u16 player = city->get_owner();
    GAME_EXPECT(player < state.m_player_n, "CityTurnHandler_Default pick_prod player out of bounds");
...
void CityTurnHandler_Default::handle (CityTurnHandlerCtx& ctx) {
    GAME_EXPECT(ctx.m_state != nullptr, "CityTurnHandler_Default got nullptr state");
    GAME_EXPECT(ctx.m_city != nullptr, "CityTurnHandler_Default got nullptr city");
