...
static u32 cheb_riv (u16 x0, u16 y0, u16 x1, u16 y1) {
    const u32 dx = (x0 > x1) ? static_cast<u32>(x0 - x1) : static_cast<u32>(x1 - x0);
    const u32 dy = (y0 > y1) ? static_cast<u32>(y0 - y1) : static_cast<u32>(y1 - y0);
    return (dx > dy) ? dx : dy;
}

bool ExploreAi::is_riv (u16 x, u16 y) const {
    return m_map.get_river(x, y) != 0u;
}

void ExploreAi::note_riv (u16 ax, u16 ay) {
    if (!is_riv(ax, ay)) {
        return;
    }
    const u32 d = cheb_riv(ax, ay, m_x, m_y);
    if (!m_riv_spot || d < cheb_riv(m_riv_x, m_riv_y, m_x, m_y)) {
        m_riv_spot = true;
        m_riv_x = ax;
        m_riv_y = ay;
    }
...
void ExploreAi::disc_tile (u16 x, u16 y) {
    note_riv(x, y);
}

bool ExploreAi::riv_spot () const {
    return m_riv_spot;
}

u16 ExploreAi::riv_x () const {
    return m_riv_x;
}

u16 ExploreAi::riv_y () const {
    return m_riv_y;
}

void ExploreAi::clr_riv_spot () {
    m_riv_spot = false;
}
...
void ExploreDistantMk1::move (u16 moves) {
    ...
    u16 lx = m_x;
    u16 ly = m_y;
    const u8 lim0 = reveal(this, m_map, m_exp, m_x, m_y, m_sight, m_player, &lx, &ly);
    ...
    for (u16 m = 0; m < moves; ++m) {
        if (st->m_phase == EDM1_PHASE_FOLLOW) {
            if (step_follow_edge(st, m_map, m_x, m_y)) {
                st->m_follow_n++;
                reveal(this, m_map, m_exp, m_x, m_y, m_sight, m_player, &lx, &ly);
        ...
        if (st->m_phase == EDM1_PHASE_GO) {
            ...
            lx = m_x;
            ly = m_y;
            const u8 lim = reveal(this, m_map, m_exp, m_x, m_y, m_sight, m_player, &lx, &ly);
            if (lim != EDM1_LIM_NONE) {
