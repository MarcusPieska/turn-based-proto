...
static const u32 G_BLEND_A = 128u;
        ...
        for (u16 x = 0; x < w; ++x) {
            ...
            if (map.get_ai_ov_intent(x, y) == intent) {
                if (solid) {
                    ...
                    blend_rgb(&r, &g, &b, tr, tg, tb, G_BLEND_A);
                }
            }
...
static bool fort_on_res (const GameArraySimple& map) {
    const u16 w = map.width();
    const u16 h = map.height();
    for (u16 y = 0; y < h; ++y) {
        for (u16 x = 0; x < w; ++x) {
...
int main (int argc, char* argv[]) {
    ...
    note_result(rslt.m_city_n > 0u, "city_n > 0");
    if (starts.n > 0u) {
        note_result(rslt.m_ord_n > 0u, "ord_n > 0");
        note_result(gah.order().ok() && gah.order().pn() == starts.n, "order pn matches starts");
    }
    note_result(rslt.m_mtn_pass_n > 0u, "mtn_pass_n > 0");
    note_result(rslt.m_bn_fort_n > 0u, "bn_fort_n > 0");
    note_result(!fort_on_res(map), "no FORT on resource tiles");
    note_result(count_intent(map, AI_TILE_OV_INTENT_CITY) == rslt.m_city_n, "city stamp count");
    note_result(count_intent(map, AI_TILE_OV_INTENT_MTN_PASS) == rslt.m_mtn_pass_n, "mtn_pass stamp count");
    note_result(count_intent(map, AI_TILE_OV_INTENT_FORT) > 0u, "some forts stamped");
    note_result(count_intent(map, AI_TILE_OV_INTENT_FORT) <= rslt.m_mtn_pass_fort_n + rslt.m_bn_fort_n, "fort stamp count");
    std::printf(
        "build: city=%u ord=%u mtn_pass=%u pass_fort=%u bn_fort=%u  wall_ms=%.3f\n",
        rslt.m_city_n,
        rslt.m_ord_n,
        rslt.m_mtn_pass_n,
        rslt.m_mtn_pass_fort_n,
        rslt.m_bn_fort_n,
        build_ms);

    char ppm_path[384];
    std::snprintf(ppm_path, sizeof(ppm_path), "%s/gen_ai_helpers_city.ppm", g_dir);
    note_result(write_stamp_ppm(ppm_path, map, AI_TILE_OV_INTENT_CITY, 0u, 255u, 255u, false), ppm_path);
    std::printf("wrote %s\n", ppm_path);

    std::snprintf(ppm_path, sizeof(ppm_path), "%s/gen_ai_helpers_mtn_pass.ppm", g_dir);
    note_result(write_stamp_ppm(ppm_path, map, AI_TILE_OV_INTENT_MTN_PASS, 255u, 0u, 0u, false), ppm_path);
    std::printf("wrote %s\n", ppm_path);

    std::snprintf(ppm_path, sizeof(ppm_path), "%s/gen_ai_helpers_fort.ppm", g_dir);
    note_result(write_stamp_ppm(ppm_path, map, AI_TILE_OV_INTENT_FORT, 255u, 255u, 0u, true), ppm_path);
    std::printf("wrote %s\n", ppm_path);

    WhiteboardMng::terminate();

    std::printf("=======================================================\n");
    std::printf(" GEN AI HELPERS: TOTAL FAILURES: %d/%d\n", total_test_fails, total_tests_run);
    std::printf("=======================================================\n");
