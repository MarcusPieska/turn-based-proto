...
static const char* G_OUT_DIR = "/home/w/Projects/simple-map-gen/anchored-farmer-test";
...
int main () {
    ...
    if (!AnchoredFarmer::begin(state)) {
        std::printf("fail anchored farmer begin\n");
        return 1;
    }
    char path[512];
    u64 handle_ns = 0;
    ...
    for (u32 t = 1; t <= G_TURNS; ++t) {
        for (u16 i = 0; i < g_wk_n; ++i) {
            ...
            AnchoredFarmer::handle(state, g_wk[i]);
