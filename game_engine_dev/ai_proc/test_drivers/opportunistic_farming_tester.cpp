...
static const char* G_OUT_DIR = "/home/w/Projects/simple-map-gen/opportunistic-farming-test";
static const u32 G_SEED = 43u;
static const u16 G_CITY_MAX = 5000;
static const u16 G_WORKER_MAX = 500;
...
static u32 count_farms (const GameState& state) {
    u32 farm_n = 0;
    const u16 w = state.m_map.width();
    const u16 h = state.m_map.height();
    for (u16 y = 0; y < h; ++y) {
...
int main () {
    ...
    g_wk_n = 0;
    const u16 want_wk = static_cast<u16>((city_n + 9u) / 10u);
    const u16 wk_cap = want_wk < G_WORKER_MAX ? want_wk : G_WORKER_MAX;
    for (u16 i = 0; i < city_n && g_wk_n < wk_cap; i = static_cast<u16>(i + 10u)) {
    ...
    if (!OpportunisticFarming::begin(state)) {
        std::printf("fail opportunistic farming begin\n");
        return 1;
    }
    char path[512];
    u64 handle_ns = 0;
    ...
    std::printf("handle calls=%llu total_ms=%.2f avg_ns=%.2f\n",
        ...
        total_ms,
        avg_ns);
    std::printf("done city_count=%u workers=%u farms=%u\n",
        (unsigned)city_n, (unsigned)g_wk_n, (unsigned)count_farms(state));
    state.clear();
    return 0;
}
