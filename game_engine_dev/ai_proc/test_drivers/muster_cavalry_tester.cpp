...
static const cstr G_OUT = "/home/w/Projects/simple-map-gen/conduct-muster-cavalry";
...
int main (int argc, char** argv) {
    ...
    Whiteboard_2B ov_wb("muster_cavalry_tester", "ov", 0u);
    if (!ov_wb.ok()) {
    ...
    if (!TestHlpUnitSpawning::spawn_at_cities(state, pa, "Spearman", 1u)
        || !TestHlpUnitSpawning::spawn_at_cities(state, pa, "Horseman", 5u)) {
    ...
    Whiteboard_1B msk("muster_cavalry_tester", "msk", 0u);
    ...
    std::printf("*** PASSED muster_cavalry (1 spear, 5 horseman)\n");
    WhiteboardMng::terminate();
    return 0;
}
