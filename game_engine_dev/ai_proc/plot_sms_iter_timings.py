...
def summarize(name: str, vals: list[float]) -> None:
    ...
    print(
        f"{name}: n={n} total={total:.3f} us avg={avg:.3f} us "
        f"early10%={early:.3f} us late10%={late:.3f} us late/early={ratio:.3f} "
        f"last={vals[-1]:.3f} us"
    )


def main() -> int:
    ap = argparse.ArgumentParser(description="Plot SenseMakingSettlerPt iter timings over time")
    ap.add_argument(
        "timings",
        nargs="?",
        default="/home/w/Projects/simple-map-gen/ai-settle-pts/iter-m200-n100/sms_iter_timings.txt",
        help="sms_iter_timings.txt path",
    )
    ...
    out = Path(args.out) if args.out else path.with_name("sms_iter_timings.png")
