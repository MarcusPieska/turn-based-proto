...
static void pick_prod (GameState& state, u16 city_idx, City* city) {
    GAME_EXPECT(city != nullptr, "CityTurnHandler_Scientific pick_prod got nullptr city");
    GAME_EXPECT(state.m_statics != nullptr, "CityTurnHandler_Scientific pick_prod got nullptr statics");
    GAME_EXPECT(state.m_player_states != nullptr, "CityTurnHandler_Scientific pick_prod got nullptr player states");

    const u16 player = city->get_owner();
    GAME_EXPECT(player < state.m_player_n, "CityTurnHandler_Scientific pick_prod player out of bounds");
    ...
    if (blds != nullptr) {
        ...
        const u16 pick = BuildingTraitOrderings::pick(*blds, static_cast<u16>(CivTrait::Scientific));
...
void CityTurnHandler_Scientific::handle (CityTurnHandlerCtx& ctx) {
    GAME_EXPECT(ctx.m_state != nullptr, "CityTurnHandler_Scientific got nullptr state");
    GAME_EXPECT(ctx.m_city != nullptr, "CityTurnHandler_Scientific got nullptr city");
