...
static void pick_prod (GameState& state, u16 city_idx, City* city) {
    GAME_EXPECT(city != nullptr, "CityTurnHandler_Militaristic pick_prod got nullptr city");
    GAME_EXPECT(state.m_statics != nullptr, "CityTurnHandler_Militaristic pick_prod got nullptr statics");
    GAME_EXPECT(state.m_player_states != nullptr, "CityTurnHandler_Militaristic pick_prod got nullptr player states");

    const u16 player = city->get_owner();
    GAME_EXPECT(player < state.m_player_n, "CityTurnHandler_Militaristic pick_prod player out of bounds");
    ...
    if (blds != nullptr) {
        ...
        const u16 pick = BuildingTraitOrderings::pick(*blds, static_cast<u16>(CivTrait::Militaristic));
...
void CityTurnHandler_Militaristic::handle (CityTurnHandlerCtx& ctx) {
    GAME_EXPECT(ctx.m_state != nullptr, "CityTurnHandler_Militaristic got nullptr state");
    GAME_EXPECT(ctx.m_city != nullptr, "CityTurnHandler_Militaristic got nullptr city");
