...
static const char* G_OUT_DIR = "/home/w/Projects/simple-map-gen/point-seq-walker-test";
static const char* G_OUT_PPM = "/home/w/Projects/simple-map-gen/point-seq-walker-test/01_walk.ppm";
...
int main () {
    ...
    for (u16 i = 0; i < net.sector_n(); ++i) {
        if (net.get(i)->m_mask == 0) {
            continue;
        }
        const Sector* a = net.get(i);
        if (!land_ok(terr[static_cast<u32>(a->m_y) * static_cast<u32>(w) + a->m_x])) {
            continue;
        }
        for (u16 j = static_cast<u16>(i + 17u); j < net.sector_n(); j = static_cast<u16>(j + 17u)) {
            if (net.get(j)->m_mask == 0) {
                continue;
            }
            const Sector* b = net.get(j);
            if (!land_ok(terr[static_cast<u32>(b->m_y) * static_cast<u32>(w) + b->m_x])) {
                continue;
            }
            SectorPath path;
            ++try_n;
            if (!router.find(i, j, &path)) {
                continue;
            }
            ++ok_n;
            const u32 d = dist2(a->m_x, a->m_y, b->m_x, b->m_y);
            if (d > best_d) {
                best_d = d;
                src = i;
                dst = j;
    ...
    log_ms("pick_farthest_pair", t0, Steady::now());
    std::printf("pick_farthest_pair tries=%u ok=%u\n", try_n, ok_n);
    std::fflush(stdout);

    if (src == U16_KEY_NULL) {
        std::printf("no route pair\n");
        std::fflush(stdout);
        delete[] terr;
        return -1;
    }
