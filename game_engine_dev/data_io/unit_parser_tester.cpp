...
void print_item (const UnitStaticDataStruct& item) {
    printf("name: %s\n", item.name.c_str());
    print_u16_member("type", item.type);
    print_u32_member("cost", item.cost);
    print_u16_member("attack", item.attack);
    print_u16_member("defense", item.defense);
    print_u16_member("mvt_pts", item.mvt_pts);
...
int run_parse_driver () {
    ...
    raw_items.load_file_content("../game_config.units");
