//================================================================================================================================
//=> - Include guards -
//================================================================================================================================

#ifndef WONDER_DATA_H
#define WONDER_DATA_H

#include <string>
#include <iosfwd>
#include <cstdint>

#include "game_primitives.h"

//================================================================================================================================
//=> - Forward declarations and constants -
//================================================================================================================================

class BitArrayCL;

static const u32 MAX_WONDER_REQS = 4;

//================================================================================================================================
//=> - Requirement enums and structs -
//================================================================================================================================

enum WonderReqType : u16 {
    WONDER_REQ_NONE = 0,
    WONDER_REQ_FLAG = 1,
    WONDER_REQ_RESOURCE = 2,
    WONDER_REQ_BUILDING = 3
};


struct WonderReqFlag {
    u16 flag_idx;
};

struct WonderReqResource {
    u16 resource_idx;
};

struct WonderReqBuilding {
    u16 building_idx;
    u16 count_required;
};

struct WonderRequirement {
    WonderReqType type;
    union {
        WonderReqFlag flag_req;
        WonderReqResource resource_req;
        WonderReqBuilding building_req;
    } data;
};

//================================================================================================================================
//=> - WonderSmallTypeStats struct -
//================================================================================================================================

struct WonderTypeStats {
    std::string name;
    u32 cost;
    u16 tech_prereq_idx;
    WonderRequirement requirements[MAX_WONDER_REQS];
};

//================================================================================================================================
//=> - WonderSmallData class -
//================================================================================================================================

class WonderData {
public:
    static void load_static_data (const std::string& filename);
    static void print_content ();
    static u16 get_wonder_data_count ();
    static const WonderTypeStats* get_wonder_data_array ();

private:
    static u16 validate_and_count (const std::string& filename);
    static void parse_and_allocate (const std::string& filename);

    WonderData () = delete;
    WonderData (const WonderData& other) = delete;
    WonderData (WonderData&& other) = delete;
};

#endif // WONDER_DATA_H

//================================================================================================================================
//=> - End of file -
//================================================================================================================================

