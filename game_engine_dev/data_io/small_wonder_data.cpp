...
void SmallWonderData::print_content () {
    if (wonder_data_array == nullptr || wonder_data_count == 0) {
        printf("Wonder-small data not loaded.\n");
        return;
    }
...
u16 SmallWonderData::get_wonder_data_count () {
    return static_cast<u16>(wonder_data_count);
}

const SmallWonderTypeStats* SmallWonderData::get_wonder_data_array () {
...
void SmallWonderData::parse_and_allocate (const std::string& filename) {
    ...
    int count = SmallWonderData::validate_and_count(filename);
    if (count == 0) {
        return;
    }
    ...
    for (size_t i = 0; i < lines.size() && idx < wonder_data_count; ++i) {
        ...
        // Convert tech prerequisite name to a static index in TechData.
