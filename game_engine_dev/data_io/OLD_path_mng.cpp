...
const std::string& PathMng::get_path_to_unit_abilities () const {
    return m_path_unit_abilities;
}
...
void PathMng::build_paths () {
    ...
    m_path_unit_abilities = m_path_offset + "/game_config.unit_abilities";
...
void PathMng::validate_paths_or_exit () const {
    ...
    if (!does_file_exist(m_path_unit_abilities)) {
        printf("ERROR: Missing file: %s\n", m_path_unit_abilities.c_str());
        ++error_count;
    }
