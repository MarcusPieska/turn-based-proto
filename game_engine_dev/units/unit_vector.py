#================================================================================================================================#
#=> - Imports -
#================================================================================================================================#

import sys
sys.dont_write_bytecode = True

import tkinter as tk
import ctypes
from ctypes import Structure, c_char, c_int, POINTER, CDLL

#================================================================================================================================#
#=> - Ctypes setup -
#================================================================================================================================#

class UnitDataC(Structure):
    _fields_ = [
        ("name", c_char * 256),
        ("cost", c_int),
        ("attack", c_int),
        ("defense", c_int),
        ("moves", c_int),
        ("strength", c_int),
    ]

try:
    lib = CDLL("./libunit_vector.so")
except OSError:
    try:
        lib = CDLL("./unit_vector.dll")
    except OSError:
        lib = CDLL("./libunit_vector.dylib")

lib.InitUnitData.argtypes = [ctypes.c_char_p]
lib.InitUnitData.restype = None

lib.DestroyUnitVector.argtypes = []
lib.DestroyUnitVector.restype = None

lib.GetUnitCount.argtypes = []
lib.GetUnitCount.restype = c_int

lib.GetUnit.argtypes = [c_int, POINTER(UnitDataC)]
lib.GetUnit.restype = None

lib.SetAvailable.argtypes = [c_int]
lib.SetAvailable.restype = None

lib.ValidateAndCount.argtypes = [ctypes.c_char_p]
lib.ValidateAndCount.restype = c_int

#================================================================================================================================#
#=> - Helpers -
#================================================================================================================================#

def print_bit_array(built_flags, available_flags, count):
    print("Built flags:")
    bits_str = ""
    for i in range(count):
        bits_str += "1" if built_flags[i] else "0"
        if (i + 1) % 10 == 0:
            bits_str += " "
        if (i + 1) % 50 == 0:
            print(bits_str)
            bits_str = ""
    if bits_str:
        print(bits_str)
    
    print("\nAvailable flags:")
    bits_str = ""
    for i in range(count):
        bits_str += "1" if available_flags[i] else "0"
        if (i + 1) % 10 == 0:
            bits_str += " "
        if (i + 1) % 50 == 0:
            print(bits_str)
            bits_str = ""
    if bits_str:
        print(bits_str)
    print()

#================================================================================================================================#
#=> - Class: UnitVectorGUI -
#================================================================================================================================#

class UnitVectorGUI:

    def __load_unit_data(m):
        filename = "../game_config.units"
        lib.InitUnitData(filename.encode('utf-8'))
        m.refresh_units()

    def __toggle_available(m, index):
        lib.SetAvailable(index)
        if index < len(m.available_states):
            m.available_states[index] = not m.available_states[index]
        m.refresh_units()

    def refresh_units(m):
        count = lib.GetUnitCount()
        for i in range(count):
            data = UnitDataC()
            lib.GetUnit(i, ctypes.byref(data))
            name = data.name.decode('utf-8')
            strength = data.strength
            
            if i < len(m.unit_rows):
                row = m.unit_rows[i]
                row["name_label"].config(text=name)
                row["stats_label"].config(text="A:%d D:%d M:%d" % (data.attack, data.defense, data.moves))
                row["strength_label"].config(text="S:%d" % strength)
                if i < len(m.available_states):
                    row["available_light"].config(bg="green" if m.available_states[i] else "red")
            else:
                row_frame = tk.Frame(m.scroll_frame)
                row_frame.pack(fill=tk.X, padx=5, pady=2)
                
                name_label = tk.Label(row_frame, text=name, width=30, anchor="w")
                name_label.pack(side=tk.LEFT, padx=5)
                
                stats_label = tk.Label(row_frame, text="A:%d D:%d M:%d" % (data.attack, data.defense, data.moves), width=15)
                stats_label.pack(side=tk.LEFT, padx=5)
                
                strength_label = tk.Label(row_frame, text="S:%d" % strength, width=8)
                strength_label.pack(side=tk.LEFT, padx=5)
                
                available_btn = tk.Button(row_frame, text="Toggle Available", 
                                        command=lambda idx=i: m.__toggle_available(idx), width=15)
                available_btn.pack(side=tk.LEFT, padx=5)
                
                available_light = tk.Label(row_frame, text="A", width=3, bg="red")
                available_light.pack(side=tk.LEFT, padx=5)
                
                m.unit_rows.append({
                    "name_label": name_label,
                    "stats_label": stats_label,
                    "strength_label": strength_label,
                    "available_light": available_light,
                })
                if i >= len(m.available_states):
                    m.available_states.append(False)
        
        while len(m.unit_rows) > count:
            m.unit_rows[-1]["name_label"].master.destroy()
            m.unit_rows.pop()
            if len(m.available_states) > count:
                m.available_states.pop()

    def __init__(m, root):
        m.root = root
        m.unit_rows = []
        m.available_states = []
        
        m.top_frame = tk.Frame(root)
        m.top_frame.pack(side=tk.TOP, fill=tk.X, padx=5, pady=5)
        
        load_data_btn = tk.Button(m.top_frame, text="Load Unit Data", 
                                  command=m.__load_unit_data, width=18)
        load_data_btn.pack(side=tk.LEFT, padx=5)
        
        m.canvas = tk.Canvas(root)
        m.scrollbar = tk.Scrollbar(root, orient="vertical", command=m.canvas.yview)
        m.scroll_frame = tk.Frame(m.canvas)
        
        m.scroll_frame.bind(
            "<Configure>",
            lambda e: m.canvas.configure(scrollregion=m.canvas.bbox("all"))
        )
        
        m.canvas.create_window((0, 0), window=m.scroll_frame, anchor="nw")
        m.canvas.configure(yscrollcommand=m.scrollbar.set)
        
        m.canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5, pady=5)
        m.scrollbar.pack(side=tk.RIGHT, fill=tk.Y, padx=5, pady=5)
        
        m.__load_unit_data()

#================================================================================================================================#
#=> - Main -
#================================================================================================================================#

if __name__ == "__main__":
    root = tk.Tk()
    root.title("Unit Vector Tester")
    root.geometry("900x600")
    gui = UnitVectorGUI(root)
    root.mainloop()
    lib.DestroyUnitVector()

#================================================================================================================================#
#=> - End -
#================================================================================================================================#
