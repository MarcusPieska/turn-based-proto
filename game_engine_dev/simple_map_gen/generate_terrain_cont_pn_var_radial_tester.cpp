...
static i32 run_var_radial_series (u32 seed) {
    char out_dir[256];
    std::snprintf(out_dir, sizeof(out_dir), "../../../simple-map-gen/var-radial/seed-%u", seed);
    if (!mkdirs(out_dir)) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        params.m_seed = seed;
        params.m_inner_grad_limit = v0 + (v1 - v0) * static_cast<f32>(i) / static_cast<f32>(n - 1);
        Generate_TerrainContPn gen(params);
        if (!gen.is_valid()) {
            std::printf("frame %d inner_grad_limit=%.4f failed\n", i, params.m_inner_grad_limit);
            return -1;
        }
        char path[384];
