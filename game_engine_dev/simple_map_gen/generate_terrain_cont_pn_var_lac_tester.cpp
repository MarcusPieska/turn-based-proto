...
static i32 run_var_lac_series (u32 seed) {
    char out_dir[256];
    std::snprintf(out_dir, sizeof(out_dir), "../../../simple-map-gen/var-lac/seed-%u", seed);
    if (!mkdirs(out_dir)) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        params.m_seed = seed;
        params.m_lacunarity = v0 + (v1 - v0) * static_cast<f32>(i) / static_cast<f32>(n - 1);
        Generate_TerrainContPn gen(params);
        if (!gen.is_valid()) {
            std::printf("frame %d lacunarity=%.4f failed\n", i, params.m_lacunarity);
            return -1;
        }
        char path[384];
    ...
    std::printf("wrote %d frames to %s (m_lacunarity %.4f .. %.4f)\n", n, out_dir, v0, v1);
    return 0;
}
...
i32 main (i32 argc, char* argv[]) {
    ...
    return run_var_lac_series(seed);
}
