...
static void bfs_open_land_to_river (
    u16 w,
    u16 h,
    const u8* terrain,
    const u8* river,
...
u8* Generate_DistanceToRiver::generate (const u8* terrain, u16 w, u16 h, const u8* river_overlay) {
    if (terrain == nullptr || river_overlay == nullptr || w == 0 || h == 0) {
        return nullptr;
    }
    const u32 n = static_cast<u32>(w) * static_cast<u32>(h);
    u16* dist = new u16[n];
    ...
    u16 max_dist = 0;
    bfs_open_land_to_river(w, h, terrain, river_overlay, dist, &max_dist);
    u32 land_fin = 0;
    for (u32 i = 0; i < n; ++i) {
    ...
    for (u32 i = 0; i < n; ++i) {
        ...
        if (is_water(cls) || is_mountain(cls)) {
            out[i] = 0;
            continue;
        }
        if (river_overlay[i] != 0) {
            out[i] = 0;
            continue;
        }
        const u16 d = dist[i];
        if (!is_open_land(cls) || d == k_inf) {
            out[i] = 0;
            continue;
        }
