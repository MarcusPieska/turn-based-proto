...
static const char* g_out_path = "/home/w/Projects/simple-map-gen/river-to-lake.ppm";
...
i32 test_adjust_river_to_lake_basic (u32 seed) {
    MapTerrainData map;
    ...
    Adjust_RiverToLake adj(seed);
    const clock_t t0 = clock();
    adj.adjust(td, lines->overlay, w, h);
    const clock_t t1 = clock();
    const double sec = static_cast<double>(t1 - t0) / static_cast<double>(CLOCKS_PER_SEC);
    if (!adj.is_valid()) {
        Generate_RiverLines::free_result(lines);
        std::printf("Adjust_RiverToLake failed to adjust\n");
        return -1;
    }
    std::printf("Adjust_RiverToLake adjust time: %.6f s (%u x %u)\n",
...
i32 main (i32 argc, char* argv[]) {
    ...
    return test_adjust_river_to_lake_basic(seed);
}
