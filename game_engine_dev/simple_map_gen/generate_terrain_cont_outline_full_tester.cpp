...
i32 main (i32 argc, char* argv[]) {
    ...
    params.m_fill_mode = TERR_OUTLINE_FILL_MODE_FULL;
    ...
    if (!ok || !gen.is_valid()) {
        std::printf("Generate_TerrainContOutline full failed\n");
        return -1;
    }
    std::printf("full mode time: %.6f s (%u x %u)\n", sec, gen.width(), gen.height());
    if (!gen.save_output("maps/out_map_terrain_cont_outline_full.ppm")) {
