...
Adjust_RiverToLake::Adjust_RiverToLake (u32 seed) :
    m_seed(seed),
    m_valid_adjust(false) {
}

bool Adjust_RiverToLake::adjust (u8* terrain, const u8* riv, u16 w, u16 h) {
    m_valid_adjust = false;
    if (terrain == nullptr || riv == nullptr || w < 2 || h < 2) {
        return false;
    }
...
bool Adjust_RiverToLake::is_valid () const {
