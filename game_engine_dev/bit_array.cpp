...
void BitArray32::serialize (std::ostream& os) const {
    os.write(reinterpret_cast<const char*>(&bits), sizeof(uint32_t));
}
...
BitArrayCL::BitArrayCL (uint32_t num_bits) : num_bits(num_bits) {
    num_arrays = (num_bits + MAX_BITS - 1) / MAX_BITS;
    arrays = new BitArray32[num_arrays];
}
...
int BitArrayCL::get_bit (int index) const {
    if (index < 0 || index >= static_cast<int>(num_bits)) {
        return 0;
    }
    int array_idx = index / MAX_BITS;
    int bit_idx = index % MAX_BITS;
    return arrays[array_idx].get_bit(bit_idx);
}
...
void BitArrayCL::set_bit (int index) {
    ...
    int array_idx = index / MAX_BITS;
    int bit_idx = index % MAX_BITS;
    arrays[array_idx].set_bit(bit_idx);
}
...
void BitArrayCL::clear_bit (int index) {
    ...
    int array_idx = index / MAX_BITS;
    int bit_idx = index % MAX_BITS;
    arrays[array_idx].clear_bit(bit_idx);
}

void BitArrayCL::serialize (std::ostream& os) const {
    os.write(reinterpret_cast<const char*>(&num_bits), sizeof(uint32_t));
    for (uint32_t i = 0; i < num_arrays; i++) {
        arrays[i].serialize(os);
    }
}
...
BitArrayCL* BitArrayCL::deserialize (std::istream& is) {
    ...
    for (uint32_t i = 0; i < ba->num_arrays; i++) {
        ba->arrays[i] = BitArray32::deserialize(is);
    }
    return ba;
}
