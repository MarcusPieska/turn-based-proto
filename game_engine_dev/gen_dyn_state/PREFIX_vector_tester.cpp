...
class [CLASS_NAME_PREFIX]VectorTester {
    public:
        static u16 get_count(const [CLASS_NAME_PREFIX]Vector& array) {
            return array.m_[MEMBER_TAG]_count;
        }
        static u16 get_page_count(const [CLASS_NAME_PREFIX]Vector& array) {
            return array.m_page_count;
        }
        static u16 get_head_count(const [CLASS_NAME_PREFIX]Vector& array) {
            return array.m_head_[MEMBER_TAG]_idx;
        }
    };
...
void test_first_item_zero_initialized () {
    ...
    [CLASS_NAME_PREFIX]Item* should_be_null = array.get_[MEMBER_TAG]([CLASS_NAME_PREFIX]Key::from_raw(0));
...
void test_null_key_reserved () {
    [CLASS_NAME_PREFIX]Vector array;
    [CLASS_NAME_PREFIX]Item* null_item = array.get_[MEMBER_TAG]([CLASS_NAME_PREFIX]Key::from_raw(0));
...
void test_array_unique_ids () {
    ...
    if (ok_ids) {
        u16 total = [CLASS_NAME_PREFIX]VectorTester::get_count(array);
...
void test_return_clears_existence () {
    ...
    if (item1 == nullptr) {
        ...
        array.return_[MEMBER_TAG]([CLASS_NAME_PREFIX]Key::from_raw(return_idx1));
        if (array.get_[MEMBER_TAG]([CLASS_NAME_PREFIX]Key::from_raw(return_idx1)) != nullptr) {
            ok = false;
        }
    }
    ...
    if (item2 == nullptr) {
        ...
        array.return_[MEMBER_TAG]([CLASS_NAME_PREFIX]Key::from_raw(return_idx2));
        if (array.get_[MEMBER_TAG]([CLASS_NAME_PREFIX]Key::from_raw(return_idx2)) != nullptr) {
            ok = false;
        }
    }
...
void test_return_null_is_noop () {
    ...
    array.return_[MEMBER_TAG]([CLASS_NAME_PREFIX]Key::from_raw(0xFFFF)); // should no-op as invalid

    bool ok = true;
    if ([CLASS_NAME_PREFIX]VectorTester::get_count(array) != before_count) {
        ok = false;
    }
...
void test_return_decrements_count () {
    ...
    if (item == nullptr) {
        ...
        if ([CLASS_NAME_PREFIX]VectorTester::get_count(array) != static_cast<u16>(before_count - 1)) {
            ok = false;
        }
        if ([CLASS_NAME_PREFIX]VectorTester::get_head_count(array) != before_head) {
            ok = false;
        }
        if (array.get_[MEMBER_TAG]([CLASS_NAME_PREFIX]Key::from_raw(return_idx)) != nullptr) {
            ok = false;
        }
    }
...
void test_return_does_not_change_head () {
    ...
    bool ok = ([CLASS_NAME_PREFIX]VectorTester::get_head_count(array) == before_head);
...
void test_reuse_recycled_slot_basic () {
    ...
    if (ok) {
        array.return_[MEMBER_TAG]([CLASS_NAME_PREFIX]Key::from_raw(return_idx));
        [CLASS_NAME_PREFIX]Key new_id = array.get_next_new_[MEMBER_TAG]_key();
...
void test_reuse_recycled_slot_lifo () {
    ...
    if (ok) {
        array.return_[MEMBER_TAG]([CLASS_NAME_PREFIX]Key::from_raw(1));
        array.return_[MEMBER_TAG]([CLASS_NAME_PREFIX]Key::from_raw(2));

        [CLASS_NAME_PREFIX]Key n1 = array.get_next_new_[MEMBER_TAG]_key();
        [CLASS_NAME_PREFIX]Key n2 = array.get_next_new_[MEMBER_TAG]_key();

        if (n1.value() != 2 || n2.value() != 1) {
            ok = false;
        }
        if ([CLASS_NAME_PREFIX]VectorTester::get_head_count(array) != 5) {
            ok = false;
        }
    }
...
void test_return_twice_is_noop_second_time () {
    ...
    if (item == nullptr) {
        ...
        u16 before_count = [CLASS_NAME_PREFIX]VectorTester::get_count(array);
        array.return_[MEMBER_TAG]([CLASS_NAME_PREFIX]Key::from_raw(return_idx));
        u16 after_first = [CLASS_NAME_PREFIX]VectorTester::get_count(array);
        array.return_[MEMBER_TAG]([CLASS_NAME_PREFIX]Key::from_raw(return_idx));
        u16 after_second = [CLASS_NAME_PREFIX]VectorTester::get_count(array);
...
void test_array_page_allocation () {
    ...
    u16 page_count = [CLASS_NAME_PREFIX]VectorTester::get_page_count(array);
    bool ok_pages = true;
