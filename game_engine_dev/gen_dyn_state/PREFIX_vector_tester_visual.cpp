//================================================================================================================================
//=> - [CLASS_NAME_PREFIX]Vector visual tester -
//================================================================================================================================
...
public:
    ...
    void print_state([CLASS_NAME_PREFIX]Vector& array) {
        u16 head_count = array.m_head_[MEMBER_TAG]_idx;
        ...
        for (u16 i = 0; i < limit; ++i) {
            char c = '-';
            [CLASS_NAME_PREFIX]Item* item = array.get_[MEMBER_TAG]([CLASS_NAME_PREFIX]Key::from_raw(i));
            if (item != nullptr) {
                c = '1';
...
static void print_state_and_sleep([CLASS_NAME_PREFIX]VectorTester& tester, [CLASS_NAME_PREFIX]Vector& array) {
    if (print_level > 2) {
...
static bool assert_count_increments([CLASS_NAME_PREFIX]Vector& array, u16 before) {
    (void)array.get_next_new_[MEMBER_TAG]_key();
    u16 after = [CLASS_NAME_PREFIX]VectorTester::get_count(array);
    return after == static_cast<u16>(before + 1);
}

static void test_large_return_and_reuse () {
    [CLASS_NAME_PREFIX]Vector array;
    [CLASS_NAME_PREFIX]VectorTester tester;
    ...
    for (u16 i = 1; i <= start_count; ++i) {
        u16 before = [CLASS_NAME_PREFIX]VectorTester::get_count(array);
        bool ok = assert_count_increments(array, before);
        note_result(ok, "get_next_new_item_key increments active count by 1");
    }

    u16 head_before_returns = [CLASS_NAME_PREFIX]VectorTester::get_head_count(array);
    ...
    for (u16 i = 1; i <= start_count; ++i) {
        ...
        bool ok = true;
        u16 before = [CLASS_NAME_PREFIX]VectorTester::get_count(array);
        array.return_[MEMBER_TAG]([CLASS_NAME_PREFIX]Key::from_raw(i));
        u16 after = [CLASS_NAME_PREFIX]VectorTester::get_count(array);
        ok = (after == static_cast<u16>(before - 1));

        if (ok && array.get_[MEMBER_TAG]([CLASS_NAME_PREFIX]Key::from_raw(i)) != nullptr) {
            ok = false;
        }
    ...
    note_result(([CLASS_NAME_PREFIX]VectorTester::get_count(array) == static_cast<u16>(start_count - return_count)),
                 "Active count after returns == start_count - return_count");
    note_result(([CLASS_NAME_PREFIX]VectorTester::get_head_count(array) == head_before_returns),
    ...
    note_result(([CLASS_NAME_PREFIX]VectorTester::get_count(array) == post_return_count),
