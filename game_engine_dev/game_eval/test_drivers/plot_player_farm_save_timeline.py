...
def main():
    if len(sys.argv) < 3:
        print("usage: plot_player_farm_save_timeline.py <data_dir> <eval_dir>")
        return 1
    ...
    for path in paths:
        seat, count, turns, farms = load_curve(path)
        label = path.stem
        if seat is not None and count is not None:
            label = f"seat {seat} (n={count})"
        ax.plot(turns, farms, linewidth=1.6, label=label)
    ax.set_xlabel("turn")
    ax.set_ylabel("farms")
    ax.set_title("Player farm count (save timeline)")
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=8)
    fig.tight_layout()
    eval_dir.mkdir(parents=True, exist_ok=True)
    out = eval_dir / "player_farm_save_timeline.png"
