...
static const char* G_DATA_NAME = "player-farm-save-timeline";
static const char* G_PLOT_PY =
    "/home/w/Projects/rts-proto/game_engine_dev/game_eval/test_drivers/plot_player_farm_save_timeline.py";
...
static bool wr_curve (
    cstr path, u16 seat, u32 count, const EvalPaths& paths, const PlayerFarmSaveTimeline& tl) {
    std::FILE* fp = std::fopen(path, "w");
    if (fp == nullptr) {
        return false;
    }
    std::fprintf(fp, "# seat=%u count=%u\n", static_cast<u32>(seat), count);
    std::fprintf(fp, "turn farms\n");
...
//================================================================================================================================
//=> - PlayerFarmSaveTimelineTester -
//================================================================================================================================

class PlayerFarmSaveTimelineTester : public EvalDriver {
public:
    PlayerFarmSaveTimelineTester ();
...
PlayerFarmSaveTimelineTester::PlayerFarmSaveTimelineTester ()
    : EvalDriver(make_need()) {
}

int PlayerFarmSaveTimelineTester::run () {
    const u16 player_n = paths().players();
    const u16 save_n = paths().save_turn_n();
    if (player_n == 0 || save_n < 2) {
        std::printf("player_farm_save_timeline: bad sizes players=%u saves=%u\n",
            static_cast<u32>(player_n), static_cast<u32>(save_n));
        return 1;
    }
    std::printf("player_farm_save_timeline scanning %u saves (map only, one at a time)\n",
        static_cast<u32>(save_n));
    PlayerFarmSaveTimelineMng mng;
    if (!mng.setup(player_n, save_n) || !mng.fill(paths())) {
        std::printf("player_farm_save_timeline: setup/fill failed\n");
        return 1;
    }
    mng.sort();
    char eval_dir[512];
    ...
    if (!paths().eval_dir(eval_dir, sizeof(eval_dir))
        ...
        std::printf("player_farm_save_timeline: bad out paths\n");
        return 1;
    }
    if (!mkdir_deep(eval_dir) || !mkdir_deep(data_dir)) {
        std::printf("player_farm_save_timeline: cannot mkdir under %s\n", paths().out_root());
        return 1;
    }
    ...
    for (u16 k = 0; k < pick_n; ++k) {
        ...
        if (!wr_curve(path, mng.seat(idx), mng.at(idx).count(), paths(), mng.at(idx))) {
            std::printf("player_farm_save_timeline: write failed %s\n", path);
            return 1;
        }
    ...
    if (rc != 0) {
        std::printf("player_farm_save_timeline: plot script failed rc=%d\n", rc);
        return 1;
    }
    std::printf("player_farm_save_timeline ok players=%u saves=%u curves=%u\n",
...
int main () {
    PlayerFarmSaveTimelineTester t;
    return t.go(G_PATHS);
}
