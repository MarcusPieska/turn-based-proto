...
def main():
    if len(sys.argv) < 3:
        print("usage: plot_player_unit_save_timeline.py <data_dir> <eval_dir>")
        return 1
    ...
    for path in paths:
        seat, count, turns, units = load_curve(path)
        label = path.stem
    ...
    ax.set_xlabel("turn")
    ax.set_ylabel("units")
    ax.set_title("Player army size (save timeline)")
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=8)
    fig.tight_layout()
    eval_dir.mkdir(parents=True, exist_ok=True)
    out = eval_dir / "player_unit_save_timeline.png"
