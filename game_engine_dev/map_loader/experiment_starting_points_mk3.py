...
def draw_dot(buf, w, h, cx, cy, rad, rgb):
    ...
    if rad <= 0:
        set_px(buf, w, h, int(cx), int(cy), rgb)
        return
...
def filt_prob_pts(pts, gray, w, h, min_n):
    n = len(pts)
    if n <= min_n:
        return list(pts)
    order = list(pts)
    ...
    for i, (x, y) in enumerate(order):
        ...
        d = gray_at(gray, w, h, x, y)
        if roll <= d:
            kept.append((x, y))
        elif len(kept) + rest - 1 >= min_n:
            pass
        else:
            kept.append((x, y))
    return kept
...
def run_mk3(map_path, rows, cols, rad, land_set, water_set, pick_n):
    ...
    dist_path = os.path.join(here, "DEL_mk3_00_water_dist.ppm")
    save_gray_ppm(dist_path, gray, w, h)
    pts = basic_dist(rows, cols, w, h)
    land = filt_land_pts(pts, classes, w, h, land_set)
    cand = filt_prob_pts(land, gray, w, h, pick_n)
    cand_path = os.path.join(here, "DEL_mk3_01_prob_cand.ppm")
    save_land_dist_img(cand_path, classes, cand, w, h, rad, land_set, MID_GRAY, WHITE)
    t2 = time.perf_counter()
    if len(cand) <= pick_n:
        picked = cand
        pick_dt = 0.0
        pick_note = "skipped (candidates <= target)"
    else:
        picked = pick_spaced(cand, pick_n)
        t3 = time.perf_counter()
        pick_dt = t3 - t2
        pick_note = "spaced max-min"
    pick_path = os.path.join(here, "DEL_mk3_02_pick.ppm")
    save_land_dist_img(pick_path, classes, picked, w, h, rad, land_set, MID_GRAY, WHITE)
    print("MAP:    %s  (%d x %d)" % (map_path, w, h))
    print("DIST:   max u16=%d  bfs %.4f s" % (max_d, t1 - t0))
    print("SAVED:  %s" % dist_path)
    print("PTS:    %d lattice, %d plains/hills, %d after prob cull (min %d)" % (
        len(pts), len(land), len(cand), pick_n))
    print("SAVED:  %s" % cand_path)
    print("PICK:   %d from %d candidates, %s, %.4f s" % (
        len(picked), len(cand), pick_note, pick_dt))
    print("SAVED:  %s" % pick_path)
...
if __name__ == "__main__":
    run_mk3(MAP_PATH, LATT_ROWS, LATT_COLS, DOT_R, LAND_SET, WATER_SET, PICK_N)
