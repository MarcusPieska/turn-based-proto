...
typedef struct ItemEffectBuild {
    u16 building_id;        // e.g. Granary, Barracks...
    ItemEffectsScope scope;
...
typedef struct ItemEffectEnable {
    u16 feature_id;         // e.g. DIPLOMACY, NUKES, AIR_UNIT...
    ItemEffectsScope scope;
} ItemEffectEnable;

typedef struct ItemEffectResearchTech {
    u16 tech_count;          // e.g. researchTech(1), researchTech(2)
} ItemEffectResearchTech;

typedef struct ItemEffectTrain {
    u16 unit_id;            // e.g. Crusader, Ancient Cavalry...
