...
class WonderStaticDataTester {
public:
    static WonderStaticDataKey make_key (u16 raw) {
        return WonderStaticDataKey::from_raw(raw);
    }
};

void test_set_items_updates_count () {
    static WonderStaticDataStruct items[3];
    ...
    WonderStaticData::set_items(items, 3);
    note_result(WonderStaticData::get_item_count() == 3, "set_items updates static item count");
    summarize_test_results();
}

void test_get_item_returns_expected_struct_by_key () {
    static WonderStaticDataStruct items[4];
    ...
    WonderStaticData::set_items(items, 4);

    WonderStaticDataKey key = WonderStaticDataTester::make_key(2);
    const WonderStaticDataStruct& item = WonderStaticData::get_item(key);
...
void test_get_item_returns_reference_to_backing_array () {
    static WonderStaticDataStruct items[2];
    items[0].cost = 11;  items[0].effects.indices[0] = 110;
    items[1].cost = 12;  items[1].effects.indices[0] = 120;
    WonderStaticData::set_items(items, 2);

    WonderStaticDataKey key = WonderStaticDataTester::make_key(1);
    const WonderStaticDataStruct& item = WonderStaticData::get_item(key);
...
void test_set_items_can_replace_array_and_count () {
    ...
    WonderStaticData::set_items(items_a, 2);
    note_result(WonderStaticData::get_item_count() == 2, "initial array count set");

    WonderStaticData::set_items(items_b, 5);
    note_result(WonderStaticData::get_item_count() == 5, "replacement array count set");
    WonderStaticDataKey key = WonderStaticDataTester::make_key(4);
    const WonderStaticDataStruct& item = WonderStaticData::get_item(key);
...
void test_zero_count_is_allowed () {
    static WonderStaticDataStruct items[1];
    items[0].cost = 41; items[0].effects.indices[0] = 410;
    WonderStaticData::set_items(items, 0);
    note_result(WonderStaticData::get_item_count() == 0, "zero item count can be stored");
    summarize_test_results();
}
...
int main (int argc, char* argv[]) {
    ...
    printf("=======================================================\n");
    printf(" TESTING WONDER REQ DATA: TOTAL FAILURES: %d/%d\n", total_test_fails, total_tests_run);
    printf("=======================================================\n");
