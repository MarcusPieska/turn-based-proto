...
class UnitStaticDataTester {
public:
    static UnitStaticDataKey make_key (u16 raw) {
        return UnitStaticDataKey::from_raw(raw);
    }
};
...
void test_set_items_updates_count () {
    ...
    UnitStaticData::set_items(items, 3);
    note_result(UnitStaticData::get_item_count() == 3, "set_items updates static item count");
...
void test_get_item_returns_expected_struct_by_key () {
    ...
    UnitStaticData::set_items(items, 4);

    UnitStaticDataKey key = UnitStaticDataTester::make_key(2);
    const UnitStaticDataStruct& item = UnitStaticData::get_item(key);
...
void test_get_item_returns_reference_to_backing_array () {
    ...
    UnitStaticData::set_items(items, 2);
...
void test_set_items_can_replace_array_and_count () {
    ...
    items_b[0].cost = 31; items_b[0].type = 310;
    items_b[1].cost = 32; items_b[1].type = 320;
    items_b[2].cost = 33; items_b[2].type = 330;
    items_b[3].cost = 34; items_b[3].type = 340;
    items_b[4].cost = 35; items_b[4].type = 350;

    UnitStaticData::set_items(items_a, 2);
    note_result(UnitStaticData::get_item_count() == 2, "initial array count set");

    UnitStaticData::set_items(items_b, 5);
    note_result(UnitStaticData::get_item_count() == 5, "replacement array count set");
    UnitStaticDataKey key = UnitStaticDataTester::make_key(4);
    const UnitStaticDataStruct& item = UnitStaticData::get_item(key);
...
void test_zero_count_is_allowed () {
    ...
    items[0].cost = 41; items[0].type = 410;
    UnitStaticData::set_items(items, 0);
    note_result(UnitStaticData::get_item_count() == 0, "zero item count can be stored");
    summarize_test_results();
}
...
int main (int argc, char* argv[]) {
    ...
    printf("=======================================================\n");
    printf(" TESTING UNIT REQ DATA: TOTAL FAILURES: %d/%d\n", total_test_fails, total_tests_run);
    printf("=======================================================\n");
