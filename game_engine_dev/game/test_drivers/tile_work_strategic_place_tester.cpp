...
static void clear_job_adds (GameArraySimple& map, u16 job_idx) {
    const u16 w = map.width();
    const u16 h = map.height();
    for (u16 y = 0; y < h; ++y) {
        for (u16 x = 0; x < w; ++x) {
            if (map.get_add_idx(x, y) == job_idx) {
                map.set_tile_add(x, y, U16_KEY_NULL, static_cast<u8>(BUILD_ADD_STD));
            }
        }
    }
}

static void set_all_roads (GameArraySimple& map, u8 road) {
    const u16 w = map.width();
    const u16 h = map.height();
    for (u16 y = 0; y < h; ++y) {
        for (u16 x = 0; x < w; ++x) {
            map.set_road_typ(x, y, road);
        }
    }
}

static void restore_roads (GameArraySimple& map, const std::vector<u8>& road) {
    const u16 w = map.width();
    const u16 h = map.height();
    for (u16 y = 0; y < h; ++y) {
        for (u16 x = 0; x < w; ++x) {
            map.set_road_typ(x, y, road[static_cast<u32>(y) * w + x]);
        }
    }
}
...
static u32 greedy_place (u16 job_idx, GameArraySimple& map, std::vector<u8>* mark) {
    const u16 w = map.width();
    const u16 h = map.height();
    const u8 add_typ = add_typ_for_job(job_idx);
    u32 n = 0;
    if (mark != nullptr) {
        mark->assign(static_cast<size_t>(w) * static_cast<size_t>(h), 0u);
    }
    for (u16 y = 0; y < h; ++y) {
        for (u16 x = 0; x < w; ++x) {
            ...
            map.set_tile_add(x, y, job_idx, add_typ);
            ++n;
            if (mark != nullptr) {
                (*mark)[static_cast<u32>(y) * w + x] = 1u;
            }
...
int main (int argc, char* argv[]) {
    ...
    if (total_test_fails > 0) {
        std::printf("=======================================================\n");
        std::printf(" STRATEGIC PLACE: TOTAL FAILURES: %d/%d\n", total_test_fails, total_tests_run);
        std::printf("=======================================================\n");
        return 1;
    }
    ...
    std::vector<u8> road_bak(tiles);
    for (u16 y = 0; y < map.height(); ++y) {
        for (u16 x = 0; x < map.width(); ++x) {
            road_bak[static_cast<u32>(y) * map.width() + x] = map.get_road_typ(x, y);
        }
    }
    ...
    for (u16 j = 0; j < job_n; ++j) {
        ...
        if (static_cast<WorkerJobType>(row.type) != WorkerJobType::Strategic) {
            continue;
        }
        ++strat_n;
        cstr job_nm = st.worker_job().get_name(WorkerJobStaticDataKey::from_raw(j));
        std::printf("-----------------------------------------------------------\n");
        std::printf("STRATEGIC JOB: %s (idx=%u)\n", job_nm, j);

        char ppm_v0[384];
        char ppm_v1[384];
        char ppm_p0[384];
        char ppm_p1[384];
        std::snprintf(ppm_v0, sizeof(ppm_v0), "%s/place_strat_%s_valid_roads_off.ppm", g_dir, job_nm);
        std::snprintf(ppm_v1, sizeof(ppm_v1), "%s/place_strat_%s_valid_roads_on.ppm", g_dir, job_nm);
        std::snprintf(ppm_p0, sizeof(ppm_p0), "%s/place_strat_%s_placed_roads_off.ppm", g_dir, job_nm);
        std::snprintf(ppm_p1, sizeof(ppm_p1), "%s/place_strat_%s_placed_roads_on.ppm", g_dir, job_nm);
        slug_spaces(ppm_v0);
        slug_spaces(ppm_v1);
        slug_spaces(ppm_p0);
        slug_spaces(ppm_p1);

        clear_job_adds(map, j);
        restore_roads(map, road_bak);
        {
            std::vector<u8> mark;
            const u32 n = count_ok(j, map, &mark);
            double ms = 0.0;
            double us = 0.0;
            time_assess_job(j, map, &ms, &us);
            std::printf("  [valid roads_off] n=%u  wall_ms=%.3f  us/tile=%.3f\n", n, ms, us);
            note_result(write_sites_ppm(ppm_v0, map, mark), ppm_v0);
            std::printf("    wrote %s\n", ppm_v0);
        }
        clear_job_adds(map, j);
        {
            std::vector<u8> mark;
            const u32 n = greedy_place(j, map, &mark);
            std::printf("  [placed roads_off] n=%u\n", n);
            note_result(write_sites_ppm(ppm_p0, map, mark), ppm_p0);
            std::printf("    wrote %s\n", ppm_p0);
            note_result(n > 0u, "placed roads_off > 0");
        }

        clear_job_adds(map, j);
        set_all_roads(map, ROAD_PATH);
        {
            std::vector<u8> mark;
            const u32 n = count_ok(j, map, &mark);
            double ms = 0.0;
            double us = 0.0;
            time_assess_job(j, map, &ms, &us);
            std::printf("  [valid roads_on] n=%u  wall_ms=%.3f  us/tile=%.3f\n", n, ms, us);
            note_result(write_sites_ppm(ppm_v1, map, mark), ppm_v1);
            std::printf("    wrote %s\n", ppm_v1);
        }
        clear_job_adds(map, j);
        {
            std::vector<u8> mark;
            const u32 n = greedy_place(j, map, &mark);
            std::printf("  [placed roads_on] n=%u\n", n);
            note_result(write_sites_ppm(ppm_p1, map, mark), ppm_p1);
            std::printf("    wrote %s\n", ppm_p1);
            note_result(n > 0u, "placed roads_on > 0");
        }

        clear_job_adds(map, j);
        restore_roads(map, road_bak);
    }
    note_result(strat_n >= 1u, "found Strategic jobs");
