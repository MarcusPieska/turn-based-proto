...
static void restore_ov_road (GameArraySimple& map, const std::vector<u8>& ov, const std::vector<u8>& road) {
    const u16 w = map.width();
    const u16 h = map.height();
    for (u16 y = 0; y < h; ++y) {
        for (u16 x = 0; x < w; ++x) {
            const u32 i = static_cast<u32>(y) * w + x;
            map.set_overlay(x, y, ov[i]);
            map.set_road_typ(x, y, road[i]);
        }
    }
}
...
int main (int argc, char* argv[]) {
    ...
    if (total_test_fails > 0) {
        std::printf("=======================================================\n");
        std::printf(" RESOURCE PLACE: TOTAL FAILURES: %d/%d\n", total_test_fails, total_tests_run);
        std::printf("=======================================================\n");
        return 1;
    }
    ...
    for (u16 y = 0; y < map.height(); ++y) {
        for (u16 x = 0; x < map.width(); ++x) {
            const u32 i = static_cast<u32>(y) * map.width() + x;
            ov_bak[i] = map.get_overlay(x, y);
            road_bak[i] = map.get_road_typ(x, y);
        }
    }
    ...
    for (u16 j = 0; j < job_n; ++j) {
        ...
        std::printf("-----------------------------------------------------------\n");
        std::printf("RESOURCE JOB: %s (idx=%u)\n", job_nm, j);

        char ppm0[384];
        char ppm1[384];
        char ppm2[384];
        std::snprintf(ppm0, sizeof(ppm0), "%s/place_%s_ov_default.ppm", g_dir, job_nm);
        std::snprintf(ppm1, sizeof(ppm1), "%s/place_%s_ov_clear.ppm", g_dir, job_nm);
        std::snprintf(ppm2, sizeof(ppm2), "%s/place_%s_roads.ppm", g_dir, job_nm);
        slug_spaces(ppm0);
        slug_spaces(ppm1);
        slug_spaces(ppm2);

        restore_ov_road(map, ov_bak, road_bak);
        run_mode("ov_default", j, map, ppm0);

        restore_ov_road(map, ov_bak, road_bak);
        clear_all_ov(map);
        run_mode("ov_clear", j, map, ppm1);

        restore_ov_road(map, ov_bak, road_bak);
        set_all_roads(map, ROAD_PATH);
        run_mode("roads_all", j, map, ppm2);
    }
    note_result(res_job_n >= 2u, "found Resource jobs");
    restore_ov_road(map, ov_bak, road_bak);
