...
int main (int argc, char* argv[]) {
    ...
    if (total_test_fails > 0) {
        std::printf("=======================================================\n");
        std::printf(" CLEARING PLACE: TOTAL FAILURES: %d/%d\n", total_test_fails, total_tests_run);
        std::printf("=======================================================\n");
        return 1;
    }
    ...
    for (u16 j = 0; j < job_n; ++j) {
        ...
        if (static_cast<WorkerJobType>(row.type) != WorkerJobType::Clearing) {
            continue;
        }
        ++clear_job_n;
        cstr job_nm = st.worker_job().get_name(WorkerJobStaticDataKey::from_raw(j));
        std::printf("-----------------------------------------------------------\n");
        std::printf("CLEARING JOB: %s (idx=%u)\n", job_nm, j);

        char ppm[384];
        std::snprintf(ppm, sizeof(ppm), "%s/place_clear_%s.ppm", g_dir, job_nm);
        slug_spaces(ppm);
        run_mode("ov_default", j, map, ppm);
    }
    note_result(clear_job_n >= 3u, "found Clearing jobs");
