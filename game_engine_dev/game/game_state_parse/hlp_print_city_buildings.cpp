...
#include "hlp_print_city_buildings.h"
...
static cstr bld_nm (const RuntimeStatics& st, u16 idx) {
    if (idx >= st.building().get_item_count()) {
        return "?";
    }
...
bool Helper_PrintCityBuildings::run (u32 turn, bool list) {
    if (!Helper_ParseSeed::load()) {
        std::printf("Helper_PrintCityBuildings: parse_seed load failed\n");
        return false;
    }
    RuntimeStaticLoader loader;
    if (!loader.load(G_RT_LIB_A, G_RT_DATA_A) && !loader.load(G_RT_LIB_B, G_RT_DATA_B)) {
        std::printf("Helper_PrintCityBuildings: statics load failed\n");
        return false;
    }
    ...
    if (!load_seats(players_path, &seats, &player_n)) {
        std::printf("Helper_PrintCityBuildings: players load failed: %s\n", players_path);
        return false;
    }
    CitiesDump cities = {};
    if (!load_cities(cities_path, &cities)) {
        std::printf("Helper_PrintCityBuildings: cities load failed: %s\n", cities_path);
        delete[] seats;
        return false;
    }
    ...
    for (u16 p = 0; p < player_n; ++p) {
        std::printf("player=%u civ=%s\n", p, Helper_CivNm::nm(st, seats[p].m_civ_index));
        for (u16 i = 0; i < cities.m_cn; ++i) {
            ...
            std::printf("  city=%u x=%u y=%u pop=%u food=%u prod=%u culture=%u blds=%u\n",
                rec.m_idx, rec.m_x, rec.m_y, rec.m_pop, rec.m_food, rec.m_prod, rec.m_culture, bn);
            if (!list) {
                continue;
            }
            for (u16 b = 0; b < bld_n; ++b) {
                if (!bank_on(cities.m_bld, rec.m_idx, b)) {
                    continue;
                }
                std::printf("      %s\n", bld_nm(st, b));
            }
