...
static bool compute_food_store_for_city (
    GameState& state,
    u16 city_idx,
    bool with_military_granary,
    u16 military_granary_bld_idx,
    u16* out_food_store
) {
    if (state.m_statics == nullptr || state.m_player_states == nullptr || out_food_store == nullptr) {
        return false;
    }
    ...
    if (with_military_granary) {
        state.m_cities.set_building_flag(city_idx, military_granary_bld_idx);
    }

    CityTileManager::maximize_food(player, city_idx);
    const TotalTileYield yld = CityTileManager::gather_yields(player, city_idx);
    const u16 worked_food = static_cast<u16>(yld.m_food > 65535u ? 65535u : yld.m_food);

    const u16 san_boost = city->get_city_sanitation_boost(city_idx);
    const i16 net_san = city->get_city_net_sanitation(san_boost);
    if (net_san < 0) {
        return false;
    }

    city->add_food(city_idx, worked_food, net_san);
    *out_food_store = city->get_current_food_store();
    return true;
}
...
int main () {
    ...
    const u16 military_granary_bld_idx = find_bld_idx(*st, "Military Granary");
    if (military_granary_bld_idx == U16_KEY_NULL) {
        std::printf("Military Granary not found\n");
        probe.clear();
        return 1;
    }
    ...
    for (u16 y = G_LATT_ORIG; pick_x == U16_KEY_NULL && y < map_h; y = static_cast<u16>(y + G_LATT_STEP)) {
        for (u16 x = G_LATT_ORIG; pick_x == U16_KEY_NULL && x < map_w; x = static_cast<u16>(x + G_LATT_STEP)) {
            ...
            city->set_population(1u);
            CityTileManager::maximize_food(city->get_owner(), 0);
            const TotalTileYield yld = CityTileManager::gather_yields(city->get_owner(), 0);
            const u16 worked_food = static_cast<u16>(yld.m_food > 65535u ? 65535u : yld.m_food);
            const u16 center_food = TileYields::get(city->get_x(), city->get_y()).m_food;
            const u32 total_food = static_cast<u32>(worked_food) + static_cast<u32>(center_food);
            const i32 food_surplus = static_cast<i32>(total_food) - 2;
            if (food_surplus < 1 || food_surplus > 18) {
                state.clear();
                continue;
            }
            const u16 san_boost = city->get_city_sanitation_boost(0);
            const i16 net_san = city->get_city_net_sanitation(san_boost);
            if (net_san < 0) {
                state.clear();
                continue;
    ...
    u16 store_base = 0;
    u16 store_with = 0;
    if (!compute_food_store_for_city(base, 0, false, military_granary_bld_idx, &store_base)) {
    ...
    if (!compute_food_store_for_city(with, 0, true, military_granary_bld_idx, &store_with)) {
    ...
    if (store_with + 1u != store_base) {
        std::printf("FAIL: expected military granary to reduce food store by 1\n");
        base.clear();
        with.clear();
        probe.clear();
        return 1;
