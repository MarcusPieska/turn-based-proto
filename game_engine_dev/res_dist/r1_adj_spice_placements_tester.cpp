...
i32 test_r1_adj_spice_placements (P1_TesterHarness& h) {
    if (!h.run_input()) {
        std::printf("P1_MakeMap input failed for spice placements\n");
        return -1;
    }
    ...
    if (chain.m_terrain == nullptr || chain.m_w == 0 || chain.m_h == 0) {
        std::printf("invalid chain input for spice placements\n");
        return -1;
    }
    ...
    std::printf("R1_Adj_SpicePlacements time: %.2f ms (spcs=%u secs=%u asg=%u plcs=%u)\n",
        ms, (unsigned)adj.spc_n(), (unsigned)adj.sec_n(), (unsigned)adj.asg_n(), adj.plc_n());
    ...
    for (u16 gi = 0; gi < adj.spc_n(); ++gi) {
        const u16 res_i = adj.spc_at(gi);
        u32 cnt = 0;
        for (u32 i = 0; i < npx; ++i) {
            if (ov[i] == res_i) {
                cnt = cnt + 1u;
        ...
        char fname[160];
        std::snprintf(fname, sizeof(fname), "48_spc_%s.ppm", nm);
        char out_path[320];
...
i32 main (i32 argc, char* argv[]) {
    ...
    const i32 rc = test_r1_adj_spice_placements(h);
    if (rc != 0) {
        return rc;
    }
    if (!h.finish()) {
