...
static u32 met_rng_next (MetRng* r) {
    r->m_s = r->m_s * 1103515245u + 12345u;
    return (r->m_s >> 16u) & 0x7fffu;
}

static u32 met_rng_100 (MetRng* r) {
    return met_rng_next(r) % 101u;
}
...
R1_Adj_MetalPlacements::R1_Adj_MetalPlacements ()
    : m_ok(false)
    , m_w(0)
    , m_h(0)
    , m_met_n(0)
...
void R1_Adj_MetalPlacements::clr () {
    m_ok = false;
    m_w = 0;
    m_h = 0;
    m_met_n = 0;
...
bool R1_Adj_MetalPlacements::adjust (
    u16* res_ov,
    const RuntimeStatics& s,
    ResDistState& st,
    const ResPlcMapCtx& ctx,
    ...
    u32 seed)
{
    ...
    m_w = ctx.m_w;
    m_h = ctx.m_h;
    const u16 met_ty = find_met_type(s);
    if (met_ty == U16_KEY_NULL) {
        std::printf("METAL res_type not found\n");
        return false;
    }
    ...
    for (u16 i = 0; i < res_n; ++i) {
        ...
        if (m_met_n < R1_MET_PLC_CAP) {
            m_met[m_met_n] = i;
            m_met_n = static_cast<u16>(m_met_n + 1u);
        }
    }
    if (m_met_n == 0) {
        std::printf("no METAL resources found\n");
        return false;
    }
    const u32 npx = static_cast<u32>(m_w) * static_cast<u32>(m_h);
    const u16 sec_cap = (sec_n < R1_MET_PLC_CAP) ? sec_n : static_cast<u16>(R1_MET_PLC_CAP);
    R1_MetSecRec slots[R1_MET_PLC_CAP];
    u8 have[R1_MET_PLC_CAP];
    ...
    if (m_sec_n >= m_met_n) {
        for (u16 i = 0; i < m_sec_n; ++i) {
            m_asg[m_asg_n].m_res = m_met[static_cast<u16>(i % m_met_n)];
        ...
        for (u16 i = 0; i < m_met_n; ++i) {
            const u16 si = static_cast<u16>(i % m_sec_n);
            m_asg[m_asg_n].m_res = m_met[i];
    ...
    Whiteboard_2B wb_vis("R1_Adj_MetalPlacements", "vis", 0u);
    P1_WB_CHK(wb_vis);
    ...
    Whiteboard_4B wb_pool("R1_Adj_MetalPlacements", "pool", 0u);
    P1_WB_CHK(wb_pool);
    u32* pool = wb_pool.get_iter_ptr();
    MetRng rng;
    rng.m_s = seed == 0u ? 1u : seed;
    u16 stamp = 0;
    for (u16 gi = 0; gi < m_met_n; ++gi) {
        const u16 res_i = m_met[gi];
        const ResDistStaticDataStruct* rd = res_rd(s, res_i);
        if (rd == nullptr || rd->has_plc == 0) {
            continue;
        }
        ...
        u32 placed = 0;
        while (left > 0 && rem > 0) {
            const u32 pick = met_rng_next(&rng) % rem;
            ...
            while (left > 0 && met_rng_100(&rng) < static_cast<u32>(R1_MET_CLU_PCT)) {
                u32 cand[4];
                u32 cn = 0;
                for (i32 k = 0; k < 4; ++k) {
                ...
                const u32 ni = cand[met_rng_next(&rng) % cn];
                res_ov[ni] = res_i;
                placed = placed + 1u;
                m_plc_n = m_plc_n + 1u;
                left = left - 1u;
...
bool R1_Adj_MetalPlacements::is_valid () const {
    return m_ok;
}

u16 R1_Adj_MetalPlacements::met_n () const {
    return m_met_n;
}

u16 R1_Adj_MetalPlacements::met_at (u16 i) const {
    return m_met[i];
}

u16 R1_Adj_MetalPlacements::sec_n () const {
    return m_sec_n;
}

u16 R1_Adj_MetalPlacements::asg_n () const {
    return m_asg_n;
}

const R1_MetAsgRec& R1_Adj_MetalPlacements::asg_at (u16 i) const {
    return m_asg[i];
}

u32 R1_Adj_MetalPlacements::plc_n () const {
    return m_plc_n;
}

bool R1_Adj_MetalPlacements::save_res_ppm (
    cstr path,
    const u8* terr,
    const u16* res_ov,
    u16 res_i) const
