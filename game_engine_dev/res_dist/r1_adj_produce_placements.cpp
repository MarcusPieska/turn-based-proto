...
static u32 prd_rng_next (PrdRng* r) {
...
//================================================================================================================================
//= - R1_Adj_ProducePlacements -
//================================================================================================================================

R1_Adj_ProducePlacements::R1_Adj_ProducePlacements ()
    : m_ok(false)
    , m_w(0)
    , m_h(0)
    , m_prd_n(0)
...
void R1_Adj_ProducePlacements::clr () {
    m_ok = false;
    m_w = 0;
    m_h = 0;
    m_prd_n = 0;
...
bool R1_Adj_ProducePlacements::adjust (
    u16* res_ov,
    const RuntimeStatics& s,
    ResDistState& st,
    const ResPlcMapCtx& ctx,
    ...
    u32 seed)
{
    ...
    m_w = ctx.m_w;
    m_h = ctx.m_h;
    const u16 prd_ty = find_prd_type(s);
    if (prd_ty == U16_KEY_NULL) {
        std::printf("PRODUCE res_type not found\n");
        return false;
    }
    ...
    for (u16 i = 0; i < res_n; ++i) {
        ...
        if (m_prd_n < R1_PRD_PLC_CAP) {
            m_prd[m_prd_n] = i;
            m_prd_n = static_cast<u16>(m_prd_n + 1u);
        }
    }
    if (m_prd_n == 0) {
        std::printf("no PRODUCE resources found\n");
        return false;
    }
    const u32 npx = static_cast<u32>(m_w) * static_cast<u32>(m_h);
    const u16 sec_cap = (sec_n < R1_PRD_PLC_CAP) ? sec_n : static_cast<u16>(R1_PRD_PLC_CAP);
    R1_PrdSecRec slots[R1_PRD_PLC_CAP];
    u8 have[R1_PRD_PLC_CAP];
    ...
    u16 keep_n = static_cast<u16>((static_cast<u32>(m_sec_n) * static_cast<u32>(R1_PRD_SEC_KEEP_PCT)) / 100u);
    if (keep_n == 0) {
        keep_n = 1;
    }
    if (keep_n > m_sec_n) {
    ...
    for (u16 i = 0; i < keep_n; ++i) {
        const u16 j = static_cast<u16>(i + (prd_rng_next(&cull) % static_cast<u32>(m_sec_n - i)));
        const R1_PrdSecRec tmp = m_sec[i];
        m_sec[i] = m_sec[j];
        m_sec[j] = tmp;
    }
    m_sec_n = keep_n;
    if (m_sec_n >= m_prd_n) {
        for (u16 i = 0; i < m_sec_n; ++i) {
            m_asg[m_asg_n].m_res = m_prd[static_cast<u16>(i % m_prd_n)];
        ...
        for (u16 i = 0; i < m_prd_n; ++i) {
            const u16 si = static_cast<u16>(i % m_sec_n);
            m_asg[m_asg_n].m_res = m_prd[i];
    ...
    Whiteboard_2B wb_vis("R1_Adj_ProducePlacements", "vis", 0u);
    P1_WB_CHK(wb_vis);
    ...
    Whiteboard_4B wb_pool("R1_Adj_ProducePlacements", "pool", 0u);
    P1_WB_CHK(wb_pool);
    u32* pool = wb_pool.get_iter_ptr();
    PrdRng rng;
    rng.m_s = seed == 0u ? 1u : seed;
    u16 stamp = 0;
    for (u16 gi = 0; gi < m_prd_n; ++gi) {
        const u16 res_i = m_prd[gi];
        const ResDistStaticDataStruct* rd = res_rd(s, res_i);
        if (rd == nullptr || rd->has_plc == 0) {
            continue;
        }
        ...
        u32 placed = 0;
        while (left > 0 && rem > 0) {
            const u32 pick = prd_rng_next(&rng) % rem;
...
bool R1_Adj_ProducePlacements::is_valid () const {
    return m_ok;
}

u16 R1_Adj_ProducePlacements::prd_n () const {
    return m_prd_n;
}

u16 R1_Adj_ProducePlacements::prd_at (u16 i) const {
    return m_prd[i];
}

u16 R1_Adj_ProducePlacements::sec_n () const {
    return m_sec_n;
}

u16 R1_Adj_ProducePlacements::asg_n () const {
    return m_asg_n;
}

const R1_PrdAsgRec& R1_Adj_ProducePlacements::asg_at (u16 i) const {
    return m_asg[i];
}

u32 R1_Adj_ProducePlacements::plc_n () const {
    return m_plc_n;
}

bool R1_Adj_ProducePlacements::save_res_ppm (
    cstr path,
    const u8* terr,
    const u16* res_ov,
    u16 res_i) const
