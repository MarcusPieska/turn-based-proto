...
static u32 liv_rng_next (LivRng* r) {
...
R1_Adj_LivestockPlacements::R1_Adj_LivestockPlacements ()
    : m_ok(false)
    , m_w(0)
    , m_h(0)
    , m_liv_n(0)
...
void R1_Adj_LivestockPlacements::clr () {
    m_ok = false;
    m_w = 0;
    m_h = 0;
    m_liv_n = 0;
...
bool R1_Adj_LivestockPlacements::adjust (
    u16* res_ov,
    const RuntimeStatics& s,
    ResDistState& st,
    const ResPlcMapCtx& ctx,
    ...
    u32 seed)
{
    ...
    m_w = ctx.m_w;
    m_h = ctx.m_h;
    const u16 liv_ty = find_liv_type(s);
    if (liv_ty == U16_KEY_NULL) {
        std::printf("LIVESTOCK res_type not found\n");
        return false;
    }
    ...
    for (u16 i = 0; i < res_n; ++i) {
        ...
        if (m_liv_n < R1_LIV_PLC_CAP) {
            m_liv[m_liv_n] = i;
            m_liv_n = static_cast<u16>(m_liv_n + 1u);
        }
    }
    if (m_liv_n == 0) {
        std::printf("no LIVESTOCK resources found\n");
        return false;
    }
    const u32 npx = static_cast<u32>(m_w) * static_cast<u32>(m_h);
    const u16 sec_cap = (sec_n < R1_LIV_PLC_CAP) ? sec_n : static_cast<u16>(R1_LIV_PLC_CAP);
    R1_LivSecRec slots[R1_LIV_PLC_CAP];
    u8 have[R1_LIV_PLC_CAP];
    ...
    if (m_sec_n >= m_liv_n) {
        for (u16 i = 0; i < m_sec_n; ++i) {
            m_asg[m_asg_n].m_res = m_liv[static_cast<u16>(i % m_liv_n)];
        ...
        for (u16 i = 0; i < m_liv_n; ++i) {
            const u16 si = static_cast<u16>(i % m_sec_n);
            m_asg[m_asg_n].m_res = m_liv[i];
    ...
    Whiteboard_2B wb_vis("R1_Adj_LivestockPlacements", "vis", 0u);
    P1_WB_CHK(wb_vis);
    ...
    Whiteboard_4B wb_pool("R1_Adj_LivestockPlacements", "pool", 0u);
    P1_WB_CHK(wb_pool);
    u32* pool = wb_pool.get_iter_ptr();
    LivRng rng;
    rng.m_s = seed == 0u ? 1u : seed;
    u16 stamp = 0;
    for (u16 gi = 0; gi < m_liv_n; ++gi) {
        const u16 res_i = m_liv[gi];
        const ResDistStaticDataStruct* rd = res_rd(s, res_i);
        if (rd == nullptr || rd->has_plc == 0) {
            continue;
        }
        ...
        u32 placed = 0;
        while (left > 0 && rem > 0) {
            const u32 pick = liv_rng_next(&rng) % rem;
...
bool R1_Adj_LivestockPlacements::is_valid () const {
    return m_ok;
}

u16 R1_Adj_LivestockPlacements::liv_n () const {
    return m_liv_n;
}

u16 R1_Adj_LivestockPlacements::liv_at (u16 i) const {
    return m_liv[i];
}

u16 R1_Adj_LivestockPlacements::sec_n () const {
    return m_sec_n;
}

u16 R1_Adj_LivestockPlacements::asg_n () const {
    return m_asg_n;
}

const R1_LivAsgRec& R1_Adj_LivestockPlacements::asg_at (u16 i) const {
    return m_asg[i];
}

u32 R1_Adj_LivestockPlacements::plc_n () const {
    return m_plc_n;
}

bool R1_Adj_LivestockPlacements::save_res_ppm (
    cstr path,
    const u8* terr,
    const u16* res_ov,
    u16 res_i) const
