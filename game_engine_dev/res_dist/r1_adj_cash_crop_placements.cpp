...
static u32 csh_rng_next (CshRng* r) {
...
R1_Adj_CashCropPlacements::R1_Adj_CashCropPlacements ()
    : m_ok(false)
    , m_w(0)
    , m_h(0)
    , m_csh_n(0)
...
void R1_Adj_CashCropPlacements::clr () {
    m_ok = false;
    m_w = 0;
    m_h = 0;
    m_csh_n = 0;
...
bool R1_Adj_CashCropPlacements::adjust (
    u16* res_ov,
    const RuntimeStatics& s,
    ResDistState& st,
    const ResPlcMapCtx& ctx,
    ...
    u32 seed)
{
    ...
    m_w = ctx.m_w;
    m_h = ctx.m_h;
    const u16 csh_ty = find_csh_type(s);
    if (csh_ty == U16_KEY_NULL) {
        std::printf("CASH_CROP res_type not found\n");
        return false;
    }
    ...
    for (u16 i = 0; i < res_n; ++i) {
        ...
        if (m_csh_n < R1_CSH_PLC_CAP) {
            m_csh[m_csh_n] = i;
            m_csh_n = static_cast<u16>(m_csh_n + 1u);
        }
    }
    if (m_csh_n == 0) {
        std::printf("no CASH_CROP resources found\n");
        return false;
    }
    const u32 npx = static_cast<u32>(m_w) * static_cast<u32>(m_h);
    const u16 sec_cap = (sec_n < R1_CSH_PLC_CAP) ? sec_n : static_cast<u16>(R1_CSH_PLC_CAP);
    R1_CshSecRec slots[R1_CSH_PLC_CAP];
    u8 have[R1_CSH_PLC_CAP];
    ...
    u16 keep_n = static_cast<u16>((static_cast<u32>(m_sec_n) * static_cast<u32>(R1_CSH_SEC_KEEP_PCT)) / 100u);
    if (keep_n == 0) {
        keep_n = 1;
    }
    if (keep_n > m_sec_n) {
    ...
    for (u16 i = 0; i < keep_n; ++i) {
        const u16 j = static_cast<u16>(i + (csh_rng_next(&cull) % static_cast<u32>(m_sec_n - i)));
        const R1_CshSecRec tmp = m_sec[i];
        m_sec[i] = m_sec[j];
        m_sec[j] = tmp;
    }
    m_sec_n = keep_n;
    if (m_sec_n >= m_csh_n) {
        for (u16 i = 0; i < m_sec_n; ++i) {
            m_asg[m_asg_n].m_res = m_csh[static_cast<u16>(i % m_csh_n)];
        ...
        for (u16 i = 0; i < m_csh_n; ++i) {
            const u16 si = static_cast<u16>(i % m_sec_n);
            m_asg[m_asg_n].m_res = m_csh[i];
    ...
    Whiteboard_2B wb_vis("R1_Adj_CashCropPlacements", "vis", 0u);
    P1_WB_CHK(wb_vis);
    ...
    Whiteboard_4B wb_pool("R1_Adj_CashCropPlacements", "pool", 0u);
    P1_WB_CHK(wb_pool);
    u32* pool = wb_pool.get_iter_ptr();
    CshRng rng;
    rng.m_s = seed == 0u ? 1u : seed;
    u16 stamp = 0;
    for (u16 gi = 0; gi < m_csh_n; ++gi) {
        const u16 res_i = m_csh[gi];
        const ResDistStaticDataStruct* rd = res_rd(s, res_i);
        if (rd == nullptr || rd->has_plc == 0) {
            continue;
        }
        ...
        u32 placed = 0;
        while (left > 0 && rem > 0) {
            const u32 pick = csh_rng_next(&rng) % rem;
...
bool R1_Adj_CashCropPlacements::is_valid () const {
    return m_ok;
}

u16 R1_Adj_CashCropPlacements::csh_n () const {
    return m_csh_n;
}

u16 R1_Adj_CashCropPlacements::csh_at (u16 i) const {
    return m_csh[i];
}

u16 R1_Adj_CashCropPlacements::sec_n () const {
    return m_sec_n;
}

u16 R1_Adj_CashCropPlacements::asg_n () const {
    return m_asg_n;
}

const R1_CshAsgRec& R1_Adj_CashCropPlacements::asg_at (u16 i) const {
    return m_asg[i];
}

u32 R1_Adj_CashCropPlacements::plc_n () const {
    return m_plc_n;
}

bool R1_Adj_CashCropPlacements::save_res_ppm (
    cstr path,
    const u8* terr,
    const u16* res_ov,
    u16 res_i) const
