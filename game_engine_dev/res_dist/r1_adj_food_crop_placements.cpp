...
static u32 crp_rng_next (CrpRng* r) {
...
R1_Adj_FoodCropPlacements::R1_Adj_FoodCropPlacements ()
    : m_ok(false)
    , m_w(0)
    , m_h(0)
    , m_crp_n(0)
...
void R1_Adj_FoodCropPlacements::clr () {
    m_ok = false;
    m_w = 0;
    m_h = 0;
    m_crp_n = 0;
...
bool R1_Adj_FoodCropPlacements::adjust (
    u16* res_ov,
    const RuntimeStatics& s,
    ResDistState& st,
    const ResPlcMapCtx& ctx,
    ...
    u32 seed)
{
    ...
    m_w = ctx.m_w;
    m_h = ctx.m_h;
    const u16 crp_ty = find_crp_type(s);
    if (crp_ty == U16_KEY_NULL) {
        std::printf("FOOD_CROP res_type not found\n");
        return false;
    }
    ...
    for (u16 i = 0; i < res_n; ++i) {
        ...
        if (m_crp_n < R1_CRP_PLC_CAP) {
            m_crp[m_crp_n] = i;
            m_crp_n = static_cast<u16>(m_crp_n + 1u);
        }
    }
    if (m_crp_n == 0) {
        std::printf("no FOOD_CROP resources found\n");
        return false;
    }
    const u32 npx = static_cast<u32>(m_w) * static_cast<u32>(m_h);
    const u16 sec_cap = (sec_n < R1_CRP_PLC_CAP) ? sec_n : static_cast<u16>(R1_CRP_PLC_CAP);
    R1_CrpSecRec slots[R1_CRP_PLC_CAP];
    u8 have[R1_CRP_PLC_CAP];
    ...
    if (m_sec_n >= m_crp_n) {
        for (u16 i = 0; i < m_sec_n; ++i) {
            m_asg[m_asg_n].m_res = m_crp[static_cast<u16>(i % m_crp_n)];
        ...
        for (u16 i = 0; i < m_crp_n; ++i) {
            const u16 si = static_cast<u16>(i % m_sec_n);
            m_asg[m_asg_n].m_res = m_crp[i];
    ...
    Whiteboard_2B wb_vis("R1_Adj_FoodCropPlacements", "vis", 0u);
    P1_WB_CHK(wb_vis);
    ...
    Whiteboard_4B wb_pool("R1_Adj_FoodCropPlacements", "pool", 0u);
    P1_WB_CHK(wb_pool);
    u32* pool = wb_pool.get_iter_ptr();
    CrpRng rng;
    rng.m_s = seed == 0u ? 1u : seed;
    u16 stamp = 0;
    for (u16 gi = 0; gi < m_crp_n; ++gi) {
        const u16 res_i = m_crp[gi];
        const ResDistStaticDataStruct* rd = res_rd(s, res_i);
        if (rd == nullptr || rd->has_plc == 0) {
            continue;
        }
        ...
        u32 placed = 0;
        while (left > 0 && rem > 0) {
            const u32 pick = crp_rng_next(&rng) % rem;
...
bool R1_Adj_FoodCropPlacements::is_valid () const {
    return m_ok;
}

u16 R1_Adj_FoodCropPlacements::crp_n () const {
    return m_crp_n;
}

u16 R1_Adj_FoodCropPlacements::crp_at (u16 i) const {
    return m_crp[i];
}

u16 R1_Adj_FoodCropPlacements::sec_n () const {
    return m_sec_n;
}

u16 R1_Adj_FoodCropPlacements::asg_n () const {
    return m_asg_n;
}

const R1_CrpAsgRec& R1_Adj_FoodCropPlacements::asg_at (u16 i) const {
    return m_asg[i];
}

u32 R1_Adj_FoodCropPlacements::plc_n () const {
    return m_plc_n;
}

bool R1_Adj_FoodCropPlacements::save_res_ppm (
    cstr path,
    const u8* terr,
    const u16* res_ov,
    u16 res_i) const
