...
i32 test_r1_adj_cash_crop_placements (P1_TesterHarness& h) {
    if (!h.run_input()) {
        std::printf("P1_MakeMap input failed for cash_crop placements\n");
        return -1;
    }
    ...
    if (chain.m_terrain == nullptr || chain.m_w == 0 || chain.m_h == 0) {
        std::printf("invalid chain input for cash_crop placements\n");
        return -1;
    }
    ...
    if (!sectors.generate(chain.m_terrain, chain.m_w, chain.m_h, mass.result(), h.prm().m_seed,
            static_cast<u16>(R1_RES_SECTOR_PCT_MET)) || !sectors.is_valid()) {
    ...
    std::printf("R1_Adj_CashCropPlacements time: %.2f ms (cshs=%u secs=%u asg=%u plcs=%u)\n",
        ms, (unsigned)adj.csh_n(), (unsigned)adj.sec_n(), (unsigned)adj.asg_n(), adj.plc_n());
    ...
    for (u16 gi = 0; gi < adj.csh_n(); ++gi) {
        const u16 res_i = adj.csh_at(gi);
        u32 cnt = 0;
        for (u32 i = 0; i < npx; ++i) {
            if (ov[i] == res_i) {
                cnt = cnt + 1u;
        ...
        char fname[160];
        std::snprintf(fname, sizeof(fname), "49_csh_%s.ppm", nm);
        char out_path[320];
...
i32 main (i32 argc, char* argv[]) {
    ...
    const i32 rc = test_r1_adj_cash_crop_placements(h);
    if (rc != 0) {
        return rc;
    }
    if (!h.finish()) {
