...
static u32 gem_rng_next (GemRng* r) {
    r->m_s = r->m_s * 1103515245u + 12345u;
    return (r->m_s >> 16u) & 0x7fffu;
}

static u32 gem_rng_100 (GemRng* r) {
    return gem_rng_next(r) % 101u;
}
...
R1_Adj_GemstonePlacements::R1_Adj_GemstonePlacements ()
    : m_ok(false)
    , m_w(0)
    , m_h(0)
    , m_gem_n(0)
...
void R1_Adj_GemstonePlacements::clr () {
    m_ok = false;
    m_w = 0;
    m_h = 0;
    m_gem_n = 0;
...
bool R1_Adj_GemstonePlacements::adjust (
    u16* res_ov,
    const RuntimeStatics& s,
    ResDistState& st,
    const ResPlcMapCtx& ctx,
    ...
    u32 seed)
{
    ...
    m_w = ctx.m_w;
    m_h = ctx.m_h;
    const u16 gem_ty = find_gem_type(s);
    if (gem_ty == U16_KEY_NULL) {
        std::printf("GEMSTONE res_type not found\n");
        return false;
    }
    ...
    for (u16 i = 0; i < res_n; ++i) {
        ...
        if (m_gem_n < R1_GEM_PLC_CAP) {
            m_gem[m_gem_n] = i;
            m_gem_n = static_cast<u16>(m_gem_n + 1u);
        }
    }
    if (m_gem_n == 0) {
        std::printf("no GEMSTONE resources found\n");
        return false;
    }
    const u32 npx = static_cast<u32>(m_w) * static_cast<u32>(m_h);
    const u16 sec_cap = (sec_n < R1_GEM_PLC_CAP) ? sec_n : static_cast<u16>(R1_GEM_PLC_CAP);
    R1_GemSecRec slots[R1_GEM_PLC_CAP];
    u8 have[R1_GEM_PLC_CAP];
    ...
    if (m_sec_n >= m_gem_n) {
        for (u16 i = 0; i < m_sec_n; ++i) {
            m_asg[m_asg_n].m_res = m_gem[static_cast<u16>(i % m_gem_n)];
        ...
        for (u16 i = 0; i < m_gem_n; ++i) {
            const u16 si = static_cast<u16>(i % m_sec_n);
            m_asg[m_asg_n].m_res = m_gem[i];
    ...
    Whiteboard_2B wb_vis("R1_Adj_GemstonePlacements", "vis", 0u);
    P1_WB_CHK(wb_vis);
    ...
    Whiteboard_4B wb_pool("R1_Adj_GemstonePlacements", "pool", 0u);
    P1_WB_CHK(wb_pool);
    u32* pool = wb_pool.get_iter_ptr();
    GemRng rng;
    rng.m_s = seed == 0u ? 1u : seed;
    u16 stamp = 0;
    for (u16 gi = 0; gi < m_gem_n; ++gi) {
        const u16 res_i = m_gem[gi];
        const ResDistStaticDataStruct* rd = res_rd(s, res_i);
        if (rd == nullptr || rd->has_plc == 0) {
            continue;
        }
        ...
        u32 placed = 0;
        while (left > 0 && rem > 0) {
            const u32 pick = gem_rng_next(&rng) % rem;
            ...
            m_plc_n = m_plc_n + 1u;
            left = left - 1u;
            u16 cx = static_cast<u16>(idx % static_cast<u32>(m_w));
            u16 cy = static_cast<u16>(idx / static_cast<u32>(m_w));
            const u16 sid0 = sec_ov[idx];
            while (left > 0 && gem_rng_100(&rng) < static_cast<u32>(R1_GEM_CLU_PCT)) {
                u32 cand[4];
                u32 cn = 0;
                for (i32 k = 0; k < 4; ++k) {
                    ...
                    const u32 ni = static_cast<u32>(ny) * static_cast<u32>(m_w) + static_cast<u32>(nx);
                    if (res_ov[ni] != U16_KEY_NULL || sec_ov[ni] != sid0) {
                        continue;
                    }
                    if (!res_tile_ok(ctx, s, res_i, ni)) {
                        continue;
                    }
                    cand[cn] = ni;
                    cn = cn + 1u;
                ...
                const u32 ni = cand[gem_rng_next(&rng) % cn];
                res_ov[ni] = res_i;
                placed = placed + 1u;
                m_plc_n = m_plc_n + 1u;
                left = left - 1u;
                cx = static_cast<u16>(ni % static_cast<u32>(m_w));
                cy = static_cast<u16>(ni / static_cast<u32>(m_w));
            }
        }
        st.add(res_i, placed);
    }
...
bool R1_Adj_GemstonePlacements::is_valid () const {
    return m_ok;
}

u16 R1_Adj_GemstonePlacements::gem_n () const {
    return m_gem_n;
}

u16 R1_Adj_GemstonePlacements::gem_at (u16 i) const {
    return m_gem[i];
}

u16 R1_Adj_GemstonePlacements::sec_n () const {
    return m_sec_n;
}

u16 R1_Adj_GemstonePlacements::asg_n () const {
    return m_asg_n;
}

const R1_GemAsgRec& R1_Adj_GemstonePlacements::asg_at (u16 i) const {
    return m_asg[i];
}

u32 R1_Adj_GemstonePlacements::plc_n () const {
    return m_plc_n;
}

bool R1_Adj_GemstonePlacements::save_res_ppm (
    cstr path,
    const u8* terr,
    const u16* res_ov,
    u16 res_i) const
