//================================================================================================================================
//=> - TradePostAddVector visual tester -
//================================================================================================================================
...
public:
    ...
    void print_state(TradePostAddVector& array) {
        u16 head_count = array.m_head_trade_post_add_idx;
        ...
        for (u16 i = 0; i < limit; ++i) {
            char c = '-';
            TradePostAddItem* item = array.get_trade_post_add(TradePostAddKey::from_raw(i));
            if (item != nullptr) {
                c = '1';
...
static void print_state_and_sleep(TradePostAddVectorTester& tester, TradePostAddVector& array) {
    if (print_level > 2) {
...
static bool assert_count_increments(TradePostAddVector& array, u16 before) {
    (void)array.get_next_new_trade_post_add_key();
    u16 after = TradePostAddVectorTester::get_count(array);
    return after == static_cast<u16>(before + 1);
}

static void test_large_return_and_reuse () {
    TradePostAddVector array;
    TradePostAddVectorTester tester;
    ...
    for (u16 i = 1; i <= start_count; ++i) {
        u16 before = TradePostAddVectorTester::get_count(array);
        bool ok = assert_count_increments(array, before);
        note_result(ok, "get_next_new_item_key increments active count by 1");
    }

    u16 head_before_returns = TradePostAddVectorTester::get_head_count(array);
    ...
    for (u16 i = 1; i <= start_count; ++i) {
        ...
        bool ok = true;
        u16 before = TradePostAddVectorTester::get_count(array);
        array.return_trade_post_add(TradePostAddKey::from_raw(i));
        u16 after = TradePostAddVectorTester::get_count(array);
        ok = (after == static_cast<u16>(before - 1));

        if (ok && array.get_trade_post_add(TradePostAddKey::from_raw(i)) != nullptr) {
            ok = false;
        }
    ...
    note_result((TradePostAddVectorTester::get_count(array) == static_cast<u16>(start_count - return_count)),
                 "Active count after returns == start_count - return_count");
    note_result((TradePostAddVectorTester::get_head_count(array) == head_before_returns),
    ...
    note_result((TradePostAddVectorTester::get_count(array) == post_return_count),
