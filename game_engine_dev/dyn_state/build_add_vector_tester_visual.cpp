...
public:
    ...
    void print_state(BuildAddVector& array) {
        u16 head_count = array.m_head_build_add_idx;
        ...
        for (u16 i = 0; i < limit; ++i) {
            char c = '-';
            BuildAddItem* item = array.get_build_add(BuildAddKey::from_raw(i));
            if (item != nullptr) {
                c = '1';
...
static void print_state_and_sleep(BuildAddVectorTester& tester, BuildAddVector& array) {
    if (print_level > 2) {
...
static bool assert_count_increments(BuildAddVector& array, u16 before) {
    (void)array.get_next_new_build_add_key();
    u16 after = BuildAddVectorTester::get_count(array);
    return after == static_cast<u16>(before + 1);
}

static void test_large_return_and_reuse () {
    BuildAddVector array;
    BuildAddVectorTester tester;
    ...
    for (u16 i = 1; i <= start_count; ++i) {
        u16 before = BuildAddVectorTester::get_count(array);
        bool ok = assert_count_increments(array, before);
        note_result(ok, "get_next_new_item_key increments active count by 1");
    }

    u16 head_before_returns = BuildAddVectorTester::get_head_count(array);
    ...
    for (u16 i = 1; i <= start_count; ++i) {
        ...
        bool ok = true;
        u16 before = BuildAddVectorTester::get_count(array);
        array.return_build_add(BuildAddKey::from_raw(i));
        u16 after = BuildAddVectorTester::get_count(array);
        ok = (after == static_cast<u16>(before - 1));

        if (ok && array.get_build_add(BuildAddKey::from_raw(i)) != nullptr) {
            ok = false;
        }
    ...
    note_result((BuildAddVectorTester::get_count(array) == static_cast<u16>(start_count - return_count)),
                 "Active count after returns == start_count - return_count");
    note_result((BuildAddVectorTester::get_head_count(array) == head_before_returns),
    ...
    note_result((BuildAddVectorTester::get_count(array) == post_return_count),
