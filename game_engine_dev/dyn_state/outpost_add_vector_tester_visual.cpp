//================================================================================================================================
//=> - OutpostAddVector visual tester -
//================================================================================================================================
...
public:
    ...
    void print_state(OutpostAddVector& array) {
        u16 head_count = array.m_head_outpost_add_idx;
...
static void print_state_and_sleep(OutpostAddVectorTester& tester, OutpostAddVector& array) {
    if (print_level > 2) {
...
static bool assert_count_increments(OutpostAddVector& array, u16 before) {
    (void)array.get_next_new_outpost_add_key();
    u16 after = OutpostAddVectorTester::get_count(array);
    return after == static_cast<u16>(before + 1);
}

static void test_large_return_and_reuse () {
    OutpostAddVector array;
    OutpostAddVectorTester tester;
    ...
    for (u16 i = 1; i <= start_count; ++i) {
        u16 before = OutpostAddVectorTester::get_count(array);
        bool ok = assert_count_increments(array, before);
        note_result(ok, "get_next_new_item_key increments active count by 1");
    }

    u16 head_before_returns = OutpostAddVectorTester::get_head_count(array);
    ...
    for (u16 i = 1; i <= start_count; ++i) {
        ...
        bool ok = true;
        u16 before = OutpostAddVectorTester::get_count(array);
        array.return_outpost_add(OutpostAddKey::from_raw(i));
        u16 after = OutpostAddVectorTester::get_count(array);
        ok = (after == static_cast<u16>(before - 1));

        if (ok && array.get_outpost_add(OutpostAddKey::from_raw(i)) != nullptr) {
            ok = false;
        }
    ...
    note_result((OutpostAddVectorTester::get_count(array) == static_cast<u16>(start_count - return_count)),
                 "Active count after returns == start_count - return_count");
    note_result((OutpostAddVectorTester::get_head_count(array) == head_before_returns),
    ...
    note_result((OutpostAddVectorTester::get_count(array) == post_return_count),
