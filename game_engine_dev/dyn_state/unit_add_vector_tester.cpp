...
void test_first_item_zero_initialized () {
    UnitAddVector array;
    UnitAddStruct* should_be_null = array.get_unit_add(UnitAddKey::None());
...
void test_null_key_reserved () {
    UnitAddVector array;
    UnitAddStruct* null_item = array.get_unit_add(UnitAddKey::None());
...
void test_item_size_is_nonzero () {
    bool ok = (sizeof(UnitAddStruct) > 0);
...
void test_return_clears_existence () {
    ...
    UnitAddStruct* item1 = array.get_unit_add(UnitAddKey::from_raw(return_idx1));
    if (item1 == nullptr) {
        ok = false;
    } else {
    ...
    UnitAddStruct* item2 = array.get_unit_add(UnitAddKey::from_raw(return_idx2));
    if (item2 == nullptr) {
        ok = false;
    } else {
...
void test_return_null_is_noop () {
    ...
    array.return_unit_add(UnitAddKey::None());
...
void test_return_decrements_count () {
    ...
    UnitAddStruct* item = array.get_unit_add(UnitAddKey::from_raw(return_idx));
    bool ok = true;
    if (item == nullptr) {
        ok = false;
    } else {
...
void test_reuse_recycled_slot_basic () {
    ...
    if (ok) {
        ...
        UnitAddStruct* reused = array.get_unit_add(UnitAddKey::from_raw(return_idx));
        if (reused == nullptr) {
            ok = false;
        }
    }
...
void test_reuse_recycled_slot_lifo () {
    ...
    UnitAddStruct* i1 = array.get_unit_add(UnitAddKey::from_raw(1));
    UnitAddStruct* i2 = array.get_unit_add(UnitAddKey::from_raw(2));
    ...
    if (ok) {
        ...
        if (UnitAddVectorTester::get_head_count(array) != 4) {
            ok = false;
        }
    }
...
void test_array_page_allocation () {
    ...
    if (page_count == 0) {
        ...
        for (u16 p = 0; p < page_count; ++p) {
            UnitAddStruct* page = array.get_page(p);
            if (page == nullptr) {
                ok_pages = false;
                break;
            }
