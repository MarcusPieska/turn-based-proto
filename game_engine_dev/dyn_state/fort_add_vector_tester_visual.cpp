...
public:
    ...
    void print_state(FortAddVector& array) {
        u16 head_count = array.m_head_fort_add_idx;
...
static void print_state_and_sleep(FortAddVectorTester& tester, FortAddVector& array) {
    if (print_level > 2) {
...
static bool assert_count_increments(FortAddVector& array, u16 before) {
    (void)array.get_next_new_fort_add_key();
    u16 after = FortAddVectorTester::get_count(array);
    return after == static_cast<u16>(before + 1);
}

static void test_large_return_and_reuse () {
    FortAddVector array;
    FortAddVectorTester tester;
    ...
    for (u16 i = 1; i <= start_count; ++i) {
        u16 before = FortAddVectorTester::get_count(array);
        bool ok = assert_count_increments(array, before);
        note_result(ok, "get_next_new_item_key increments active count by 1");
    }

    u16 head_before_returns = FortAddVectorTester::get_head_count(array);
    ...
    for (u16 i = 1; i <= start_count; ++i) {
        ...
        bool ok = true;
        u16 before = FortAddVectorTester::get_count(array);
        array.return_fort_add(FortAddKey::from_raw(i));
        u16 after = FortAddVectorTester::get_count(array);
        ok = (after == static_cast<u16>(before - 1));

        if (ok && array.get_fort_add(FortAddKey::from_raw(i)) != nullptr) {
            ok = false;
        }
    ...
    note_result((FortAddVectorTester::get_count(array) == static_cast<u16>(start_count - return_count)),
                 "Active count after returns == start_count - return_count");
    note_result((FortAddVectorTester::get_head_count(array) == head_before_returns),
    ...
    note_result((FortAddVectorTester::get_count(array) == post_return_count),
