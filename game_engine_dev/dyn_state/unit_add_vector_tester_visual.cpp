...
public:
    ...
    void print_state(UnitAddVector& array) {
        u16 head_count = array.m_head_unit_add_idx;
        ...
        for (u16 i = 0; i < limit; ++i) {
            char c = '-';
            UnitAddItem* item = array.get_unit_add(UnitAddKey::from_raw(i));
            if (item != nullptr) {
                c = '1';
...
static void print_state_and_sleep(UnitAddVectorTester& tester, UnitAddVector& array) {
    if (print_level > 2) {
...
static bool assert_count_increments(UnitAddVector& array, u16 before) {
    (void)array.get_next_new_unit_add_key();
    u16 after = UnitAddVectorTester::get_count(array);
    return after == static_cast<u16>(before + 1);
}

static void test_large_return_and_reuse () {
    UnitAddVector array;
    UnitAddVectorTester tester;
    ...
    for (u16 i = 1; i <= start_count; ++i) {
        u16 before = UnitAddVectorTester::get_count(array);
        bool ok = assert_count_increments(array, before);
        note_result(ok, "get_next_new_item_key increments active count by 1");
    }

    u16 head_before_returns = UnitAddVectorTester::get_head_count(array);
    ...
    for (u16 i = 1; i <= start_count; ++i) {
        ...
        bool ok = true;
        u16 before = UnitAddVectorTester::get_count(array);
        array.return_unit_add(UnitAddKey::from_raw(i));
        u16 after = UnitAddVectorTester::get_count(array);
        ok = (after == static_cast<u16>(before - 1));

        if (ok && array.get_unit_add(UnitAddKey::from_raw(i)) != nullptr) {
            ok = false;
        }
    ...
    note_result((UnitAddVectorTester::get_count(array) == static_cast<u16>(start_count - return_count)),
                 "Active count after returns == start_count - return_count");
    note_result((UnitAddVectorTester::get_head_count(array) == head_before_returns),
    ...
    note_result((UnitAddVectorTester::get_count(array) == post_return_count),
