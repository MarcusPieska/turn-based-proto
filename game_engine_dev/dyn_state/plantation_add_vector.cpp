...
PlantationAddStruct* PlantationAddVector::get_plantation_add(PlantationAddKey key) {
    if (!key.is_valid()) {
        return nullptr;
    }
...
const PlantationAddStruct* PlantationAddVector::get_plantation_add(PlantationAddKey key) const {
    if (!key.is_valid()) {
        return nullptr;
    }
    u16 plantation_add_idx = key.value();
...
PlantationAddKey PlantationAddVector::get_next_new_plantation_add_key() {
    if (m_recycled_plantation_add_count > 0) {
        ...
        if (add_page < MAX_PAGES && m_pages[add_page] && m_exists_pages[add_page]) {
            PlantationAddStruct* add = &m_pages[add_page][add_slot];
            m_exists_pages[add_page][add_slot] = 1;
            *add = PlantationAddStruct{};
    ...
    if (!m_pages[page]) {
        m_pages[page] = new PlantationAddStruct[PLANTATION_ADD_ITEMS_PER_PAGE]();
    ...
    m_pages[page][slot] = PlantationAddStruct{};
...
PlantationAddStruct* PlantationAddVector::get_page(u16 page_idx) {
...
const PlantationAddStruct* PlantationAddVector::get_page(u16 page_idx) const {
    if (page_idx >= MAX_PAGES) {
        return nullptr;
    }
    return m_pages[page_idx];
