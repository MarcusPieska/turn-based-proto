...
void test_first_item_zero_initialized () {
    ...
    ShipyardAddStruct* should_be_null = array.get_shipyard_add(ShipyardAddKey::None());
...
void test_null_key_reserved () {
    ShipyardAddVector array;
    ShipyardAddStruct* null_item = array.get_shipyard_add(ShipyardAddKey::None());
...
void test_item_size_is_nonzero () {
    bool ok = (sizeof(ShipyardAddStruct) > 0);
...
void test_return_clears_existence () {
    ...
    ShipyardAddStruct* item1 = array.get_shipyard_add(ShipyardAddKey::from_raw(return_idx1));
    if (item1 == nullptr) {
        ok = false;
    } else {
    ...
    ShipyardAddStruct* item2 = array.get_shipyard_add(ShipyardAddKey::from_raw(return_idx2));
    if (item2 == nullptr) {
        ok = false;
    } else {
...
void test_return_decrements_count () {
    ...
    ShipyardAddStruct* item = array.get_shipyard_add(ShipyardAddKey::from_raw(return_idx));
    bool ok = true;
    if (item == nullptr) {
        ok = false;
    } else {
...
void test_reuse_recycled_slot_basic () {
    ...
    if (ok) {
        ...
        ShipyardAddStruct* reused = array.get_shipyard_add(ShipyardAddKey::from_raw(return_idx));
        if (reused == nullptr) {
            ok = false;
        }
    }
...
void test_reuse_recycled_slot_lifo () {
    ...
    ShipyardAddStruct* i1 = array.get_shipyard_add(ShipyardAddKey::from_raw(1));
    ShipyardAddStruct* i2 = array.get_shipyard_add(ShipyardAddKey::from_raw(2));
...
void test_array_page_allocation () {
    ...
    if (page_count == 0) {
        ...
        for (u16 p = 0; p < page_count; ++p) {
            ShipyardAddStruct* page = array.get_page(p);
            if (page == nullptr) {
                ok_pages = false;
                break;
            }
