...
void test_first_item_zero_initialized () {
    ...
    ShipyardAddItem* should_be_null = array.get_shipyard_add(ShipyardAddKey::from_raw(0));
...
void test_null_key_reserved () {
    ShipyardAddVector array;
    ShipyardAddItem* null_item = array.get_shipyard_add(ShipyardAddKey::from_raw(0));
    ShipyardAddKey first_key = array.get_next_new_shipyard_add_key();
...
void test_item_size_is_nonzero () {
    bool ok = (sizeof(ShipyardAddItem) > 0);
...
void test_array_unique_ids () {
    ...
    if (ok_ids) {
        u16 total = ShipyardAddVectorTester::get_count(array);
        if (total < target_count) {
            ok_ids = false;
        } else {
            for (u16 i = 0; i < target_count; ++i) {
                const u16 expected = static_cast<u16>(i + 1);
                ShipyardAddItem* item = array.get_shipyard_add(ShipyardAddKey::from_raw(expected));
                if (item == nullptr) {
                    ok_ids = false;
                    break;
                }
...
void test_return_clears_existence () {
    ...
    for (u16 i = 0; i < target_count; ++i) {
        array.get_next_new_shipyard_add_key();
    }

    bool ok = true;

    const u16 return_idx1 = 3;
    ShipyardAddItem* item1 = array.get_shipyard_add(ShipyardAddKey::from_raw(return_idx1));
    if (item1 == nullptr) {
        ok = false;
    } else {
        array.return_shipyard_add(ShipyardAddKey::from_raw(return_idx1));
        if (array.get_shipyard_add(ShipyardAddKey::from_raw(return_idx1)) != nullptr) {
            ok = false;
        }
    }

    const u16 return_idx2 = 8;
    ShipyardAddItem* item2 = array.get_shipyard_add(ShipyardAddKey::from_raw(return_idx2));
    if (item2 == nullptr) {
        ok = false;
    } else {
        array.return_shipyard_add(ShipyardAddKey::from_raw(return_idx2));
        if (array.get_shipyard_add(ShipyardAddKey::from_raw(return_idx2)) != nullptr) {
            ok = false;
        }
    }
...
void test_return_null_is_noop () {
    ...
    u16 before_count = ShipyardAddVectorTester::get_count(array);
    u16 before_head  = ShipyardAddVectorTester::get_head_count(array);

    array.return_shipyard_add(ShipyardAddKey::from_raw(0xFFFF)); // should no-op as invalid

    bool ok = true;
    if (ShipyardAddVectorTester::get_count(array) != before_count) {
        ok = false;
    }
    if (ShipyardAddVectorTester::get_head_count(array) != before_head) {
        ok = false;
    }
...
void test_return_decrements_count () {
    ...
    ShipyardAddItem* item = array.get_shipyard_add(ShipyardAddKey::from_raw(return_idx));
    bool ok = true;
    if (item == nullptr) {
        ok = false;
    } else {
        array.return_shipyard_add(ShipyardAddKey::from_raw(return_idx));
        if (ShipyardAddVectorTester::get_count(array) != static_cast<u16>(before_count - 1)) {
            ok = false;
        }
        if (ShipyardAddVectorTester::get_head_count(array) != before_head) {
            ok = false;
        }
        if (array.get_shipyard_add(ShipyardAddKey::from_raw(return_idx)) != nullptr) {
            ok = false;
        }
    }
...
void test_return_does_not_change_head () {
    ...
    array.return_shipyard_add(ShipyardAddKey::from_raw(1));
    bool ok = (ShipyardAddVectorTester::get_head_count(array) == before_head);
...
void test_reuse_recycled_slot_basic () {
    ...
    if (ok) {
        array.return_shipyard_add(ShipyardAddKey::from_raw(return_idx));
        ShipyardAddKey new_id = array.get_next_new_shipyard_add_key();
        ...
        ShipyardAddItem* reused = array.get_shipyard_add(ShipyardAddKey::from_raw(return_idx));
        if (reused == nullptr) {
            ok = false;
        }
    }
...
void test_reuse_recycled_slot_lifo () {
    ...
    ShipyardAddItem* i1 = array.get_shipyard_add(ShipyardAddKey::from_raw(1));
    ShipyardAddItem* i2 = array.get_shipyard_add(ShipyardAddKey::from_raw(2));
    bool ok = (i1 != nullptr && i2 != nullptr);
    if (ok) {
        array.return_shipyard_add(ShipyardAddKey::from_raw(1));
        array.return_shipyard_add(ShipyardAddKey::from_raw(2));

        ShipyardAddKey n1 = array.get_next_new_shipyard_add_key();
        ShipyardAddKey n2 = array.get_next_new_shipyard_add_key();

        if (n1.value() != 2 || n2.value() != 1) {
            ok = false;
        }
        if (ShipyardAddVectorTester::get_head_count(array) != 5) {
            ok = false;
        }
    }
...
void test_return_twice_is_noop_second_time () {
    ...
    if (item == nullptr) {
        ...
        u16 before_count = ShipyardAddVectorTester::get_count(array);
        array.return_shipyard_add(ShipyardAddKey::from_raw(return_idx));
        u16 after_first = ShipyardAddVectorTester::get_count(array);
        array.return_shipyard_add(ShipyardAddKey::from_raw(return_idx));
        u16 after_second = ShipyardAddVectorTester::get_count(array);
...
void test_array_page_allocation () {
    ...
    u16 page_count = ShipyardAddVectorTester::get_page_count(array);
    bool ok_pages = true;
    ...
    if (page_count == 0) {
        ...
        for (u16 p = 0; p < page_count; ++p) {
            ShipyardAddItem* page = array.get_page(p);
            if (page == nullptr) {
                ok_pages = false;
                break;
            }
