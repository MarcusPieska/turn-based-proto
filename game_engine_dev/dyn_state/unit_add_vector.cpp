...
UnitAddStruct* UnitAddVector::get_unit_add(UnitAddKey key) {
    if (!key.is_valid()) {
        return nullptr;
    }
...
const UnitAddStruct* UnitAddVector::get_unit_add(UnitAddKey key) const {
    if (!key.is_valid()) {
        return nullptr;
    }
    u16 unit_add_idx = key.value();
...
UnitAddKey UnitAddVector::get_next_new_unit_add_key() {
    if (m_recycled_unit_add_count > 0) {
        ...
        if (add_page < MAX_PAGES && m_pages[add_page] && m_exists_pages[add_page]) {
            UnitAddStruct* add = &m_pages[add_page][add_slot];
...
UnitAddStruct* UnitAddVector::get_page(u16 page_idx) {
...
const UnitAddStruct* UnitAddVector::get_page(u16 page_idx) const {
    if (page_idx >= MAX_PAGES) {
        return nullptr;
    }
    return m_pages[page_idx];
